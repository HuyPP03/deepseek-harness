...
describe('jsonrpc-agent keyless smoke', () => {
  it.each([
    ...
    try {
      ...
      expect(initialized).toMatchObject({
        jsonrpc: '2.0',
        id: 1,
        result: { serverInfo: { name: 'open-harness-sdk-runtime' } },
      })
  ...
  it('rejects an invalid max-token success env value', async () => {
    ...
    expect(stderr).toContain('failed to apply loader entry sdk-jsonrpc-server (@open-harness/oh-sdk-jsonrpc-server)')
