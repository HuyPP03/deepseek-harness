...
import { boot, resolveConfigPath } from '@open-harness/oh-app-boot'
...
process.env.OH_TELEMETRY_E2E_URL = `http://127.0.0.1:${address.port}/v1/logs`
...
try {
  ...
  const mode = process.env.OH_TELEMETRY_E2E_MODE ?? 'FULL'
  if (mode !== 'FULL') {
