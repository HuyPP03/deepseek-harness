...
import { boot } from '@open-harness/oh-app-boot'
...
import type {} from '@open-harness/oh-fs-e2b'
import type {} from '@open-harness/oh-bash-local'
import type {} from '@open-harness/oh-lsp-stdio'
import type {} from '@open-harness/oh-terminal-bash'
...
try {
  ...
  const sleeping = ctx.terminals.startSend(owner, terminal.sessionId, {
    text: "printf 'OH_SLEEP_%s\\n' READY; sleep 30",
    submit: true,
  })
  let sleepReadyOutput = ''
  const sleepReadyDeadline = Date.now() + 5_000
  while (!sleepReadyOutput.includes('OH_SLEEP_READY\n')) {
    sleepReadyOutput += sleeping.readOutput().delta
    if (sleepReadyOutput.includes('OH_SLEEP_READY\n')) break
  ...
  const stubborn = await ctx.terminals.startSend(owner, terminal.sessionId, {
    text: "bash -c 'trap \"\" TERM; exec sleep 30' & printf 'OH_STUBBORN_PID=%s\\n' \"$!\"",
    submit: true,
  }).done
  const stubbornMatch = /OH_STUBBORN_PID=([1-9][0-9]*)/.exec(stubborn.viewport)
