...
import SessionStore, { SESSION_FORMAT_VERSION, SessionId, type SessionEvent, type SessionHeader } from '@open-harness/oh-session'
...
describe('parent-only override inheritance snapshot', () => {
  it('confines a delegated child through the assembled headless app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      tempDirPrefix: 'oh-subagent-inherit-',
      binScript,
      ...
      env: {
        ...
        OH_SNAPSHOT_FILE: replayOverride,
      ...
      inspect: async (runCwd) => {
        ...
        const runtimeContexts = (content: string): string[] => content.trimEnd().split('\n').flatMap((line) => {
          ...
          if (record.type !== 'user/message'
            ...
            || record.data.source.plugin !== '@open-harness/oh-system-prompt') return []
        ...
        for (const context of policyContexts) {
          expect(context).toContain('Any available operation enforced by the Open Harness file sandbox cannot modify files in the standing mode.')
