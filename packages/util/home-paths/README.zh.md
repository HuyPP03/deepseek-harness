...
`resolveOhHome()` 解析 Open Harness 的单根主目录。优先级从高到低为：显式配置的路径、`$OH_HOME`、`~/.oh`。harness 将所有用户数据保存在同一根目录下。
...
`defaultDshHome()` 使用 Node 的平台路径规则，将操作系统主目录与 `.oh` 拼接，并返回默认 Open Harness 主目录。
