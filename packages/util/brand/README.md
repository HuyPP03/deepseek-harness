...
A package brands the ids it owns — `CallId` in `oh-llm`, the shared agent/session `SessionId` in `oh-session`, and `JobId` in `oh-jobs`. Brand cross-package ids that could plausibly be confused; not every string needs one.

This package owns only the primitive. Keeping it dependency-free lets `oh-jobs`, for example, brand `JobId` without importing an unrelated capability package merely to reach `Branded`.
