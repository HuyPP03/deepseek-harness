 ...
 * @module oh-subprocess-local/spawn
 */
...
function privateSpillDir(): string {
  defaultSpillDir ??= mkdtempSync(join(tmpdir(), 'oh-subprocess-'))
...
export class OutputCollector {
  ...
  private spillAll(chunk: Buffer): void {
    ...
    if (this.spillFd === undefined) {
      ...
      this.spillFile = join(
        this.spillDir,
        `oh-subprocess-${process.pid}-${++spillCounter}-${randomBytes(6).toString('hex')}-${this.label}.log`,
      )
