...
describe('SubprocessRuntime seam', () => {
  ...
  it('scrubbedParentEnv drops credential-shaped and OH_ names (case-insensitively) but keeps PATH', () => {
    process.env.OH_SCRUB_PROBE = 'stale'
    process.env.oh_scrub_probe_lower = 'stale'
    process.env.SCRUB_PROBE_TOKEN = 'secret'
    process.env.SCRUB_PROBE_PASSWORD = 'secret'
    process.env.SCRUB_PROBE_PLAIN = 'visible'
    try {
      const env = scrubbedParentEnv()
      expect(env.OH_SCRUB_PROBE).toBeUndefined()
      expect(env.oh_scrub_probe_lower).toBeUndefined()
      ...
      delete process.env.OH_SCRUB_PROBE
      delete process.env.oh_scrub_probe_lower
