...
const spillDir = mkdtempSync(join(tmpdir(), 'oh-tool-bash-spec-'))
...
describe('the model-facing bash tool builds its request from named args only (no {...args} forward)', () => {
  const recordingDshHome = join(spillDir, 'oh-home')
   ...
   * (the credential scrub in oh-bash-local is the security control; see the
  ...
  async function setupRecording(withJsonl = false) {
    ...
    await ctx.plugin(LocalJobRegistry)
    await ctx.plugin(ToolTasks)
    await ctx.plugin(BashEnvPlugin, { ohHome: recordingDshHome })
  ...
  it('describes the managed harness environment namespace to the model', async () => {
    ...
    expect(description).toContain('$OH_*')
    expect(description).not.toContain('OH_SESSION_JSONL')
  })
  ...
  it('injects the same trusted variables into a background request without forwarding model env', async () => {
    ...
    await ctx.tools.execute({
      ...
      arguments: {
        ...
        env: { OH_SESSION_ID: 'spoofed', OH_SESSION_JSONL: '/tmp/spoofed' },
      },
      agent,
    })
  ...
  it('injects built-ins and the stable session id when no JSONL locator is available', async () => {
    ...
    const ambient = process.env.OH_SESSION_ID
    ...
    expect(process.env.OH_SESSION_ID).toBe(ambient)
  })
    ...
    expect(bash.requests.map(request => request.ohEnv)).toEqual([
      {
        ...
        OH_SESSION_JSONL: ctx.sessionPersistence.locate(parent.session.header)?.path,
        OH_SHELL: '1',
      },
      {
        OH_HOME: recordingDshHome,
        OH_SESSION_ID: 'request-child',
        OH_SESSION_JSONL: ctx.sessionPersistence.locate(child.session.header)?.path,
        OH_SHELL: '1',
      },
    ])
