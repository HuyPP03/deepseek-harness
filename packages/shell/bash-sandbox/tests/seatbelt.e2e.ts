...
import { seatbeltProfileArgs } from '@open-harness/oh-sandbox-local/src/profiles.ts'
 ...
 * confinement is covered by `@open-harness/oh-sandbox-local`. Skips off macOS or when
...
async function tempDir(base: string): Promise<string> {
  const dir = await mkdtemp(join(base, 'oh-seatbelt-e2e-'))
  tempDirs.push(dir)
  return dir
}
...
describe.skipIf(!seatbeltUsable)('bash-sandbox: real Seatbelt confinement through ctx.shell', () => {
  ...
  it('evaluates BASH_ENV only after Seatbelt confines the inner Bash', async () => {
    ...
    await writeFile(hook, [
      'printf hook > "$OH_BASH_ENV_INSIDE"',
      'printf escaped > "$OH_BASH_ENV_OUTSIDE"',
      '',
    ].join('\n'))
    const bash = await sandboxedBash(workdir, 'workspace-write')
