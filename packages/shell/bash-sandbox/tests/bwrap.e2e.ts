...
import { bwrapProfileArgs } from '@open-harness/oh-sandbox-local/src/profiles.ts'
 ...
 * backend-only confinement is covered by `@open-harness/oh-sandbox-local`.
 *
...
async function tempDir(base: string): Promise<string> {
  const dir = await mkdtemp(join(base, 'oh-bwrap-e2e-'))
  tempDirs.push(dir)
  return dir
}
