...
import type { ShellRunResult, CollectedOutput } from '@open-harness/oh-shell'
import { SANDBOX_UNAVAILABLE, SandboxProvider, SandboxUnavailableError } from '@open-harness/oh-sandbox'
import type { ConfinedArgv, SandboxExecutionPolicy, SandboxMode, SandboxPolicy } from '@open-harness/oh-sandbox'
import { SandboxPolicyService } from '@open-harness/oh-sandbox-policy'
import { SandboxBashExecutor } from '@open-harness/oh-bash-sandbox'
import LocalSubprocessRuntime from '@open-harness/oh-subprocess-local'
import type { SubprocessHandle, SubprocessOutputReader } from '@open-harness/oh-subprocess'
import { classifyDenial, classifyRunnerFailure, isRunnerSpawnFailure } from '../src/helpers.ts'
import type { Config } from '@open-harness/oh-bash-sandbox'

const spillDir = mkdtempSync(join(tmpdir(), 'oh-bash-sandbox-spec-'))
...
describe('the provider hand-off', () => {
  ...
  it('hands the provider\'s returned argv directly to ctx.subprocess.spawn', async () => {
    const returnedArgv = ['env', 'OH_WRAP=1', 'bash', '-c', 'printf "%s" "$OH_WRAP"']
    ...
    const result = await bash.run(bash.resolve({ command: 'printf "%s" "$OH_WRAP"' }))
  ...
  it('starts a non-Bash runner before the confined inner Bash evaluates BASH_ENV', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'oh-bash-env-order-'))
    ...
    const runnerScript = [
      ...
      'appendFileSync(process.env.OH_ORDER_FILE, "runner\\n");',
...
describe('fail closed', () => {
  ...
  it.each(RUNNER_FORMS)(
    ...
    async (_form, runner) => {
      ...
      const parent = mkdtempSync(join(tmpdir(), 'oh-sandbox-missing-cwd-'))
      try {
...
describe('result facts', () => {
  ...
  it('reports a real permission failure as a sandbox denial with the mode it ran under', async () => {
    const { bash } = await setup()
    const lockedDir = join(mkdtempSync(join(tmpdir(), 'oh-sandbox-denied-')), 'locked')
    mkdirSync(lockedDir)
