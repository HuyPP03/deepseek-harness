 ...
 * @module @open-harness/oh-tool-bash-persistent
 */
...
function markers(): CommandMarkers {
  ...
  return {
    start: `__OH_PERSISTENT_BASH_START_${nonce}__`,
    end: `__OH_PERSISTENT_BASH_END_${nonce}:`,
  }
}
...
function wrapCommand(command: string, marker: CommandMarkers): string {
  ...
  return `printf '%s\\n' ${quoteForBash(marker.start)}; eval -- ${quoteForBash(command)}; __oh_persistent_bash_status=$?; printf '%s%s\\n' ${quoteForBash(marker.end)} "$__oh_persistent_bash_status"`
}
