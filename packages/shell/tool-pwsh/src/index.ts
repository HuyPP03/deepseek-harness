 ...
 * `@open-harness/oh-pwsh-local`) backs `ctx.shell`; the tool contract is
 * PowerShell-dialect: native `C:\...` paths and `$env:NAME` variables.
 *
 * Behavior mirrors `oh-tool-bash` call-for-call: foreground and
 * `run_in_background` execution (background handles register with the
 * generic `ctx.jobs` runtime), the managed `OH_*` environment through the
 ...
 * exit-status parse from `@open-harness/oh-shell`.
 *
 * @module @open-harness/oh-tool-pwsh
 */

import { isAbsolute, resolve as resolvePath } from 'node:path'
import type { Context } from '@open-harness/cordis'
import z from '@open-harness/schemastery'
import { defineTool, TOOL_ABORTED } from '@open-harness/oh-tools'
import type { GenericCallView, TerminalCallView, ToolExecution, ToolResult, ToolResultView } from '@open-harness/oh-tools'
...
import { ESCALATION_TARGETS, approveEscalation, validateEscalationArgs } from '@open-harness/oh-sandbox'
import type { SandboxPolicyService } from '@open-harness/oh-sandbox-policy'
import type { ShellRunResult } from '@open-harness/oh-shell'
import { parseExitStatus } from '@open-harness/oh-shell'
import { processOutcome } from './background.ts'
import { renderPwshProcessRead, renderPwshResult } from './render.ts'
import type { RenderablePwshResult } from './render.ts'

declare module '@open-harness/oh-jobs' {
...
/* jscpd:ignore-start -- minimal mirror of oh-tool-bash's validation and execute plumbing (Agent Note). */
...
function pwshDescription(backgroundEnabled: boolean, escalationModes: readonly SandboxMode[]): string {
  ...
  const base = 'Execute a PowerShell command (`pwsh -Command`) and return its stdout/stderr. '
    ...
    + 'Current harness environment facts are exposed through managed `$env:OH_*` variables; inspect them when needed. '
...
function canonicalPwshResult(result: ShellRunResult): PwshForegroundResult {
  ...
  return {
    ...
    /* jscpd:ignore-start -- the canonical projection and background-handle shape mirror oh-tool-bash's by design (Agent Note). */
...
/* jscpd:ignore-start -- deliberate mirror of oh-tool-bash's apply() preamble (pwsh-tool-and-executor Agent Note). */
export function apply(ctx: Context, config: Config = {}): void {
  ...
  /* jscpd:ignore-start -- deliberate mirror of oh-tool-bash's escalation resolver (pwsh-tool-and-executor Agent Note). */
  /**
  ...
  ctx.tools.register(defineTool({
    ...
    /* jscpd:ignore-start -- deliberate mirror of oh-tool-bash's parameter surface (pwsh-tool-and-executor Agent Note). */
    parameters: {
    ...
    /* jscpd:ignore-end */
    output: {
      // The foreground result wire shape mirrors oh-tool-bash's by contract —
      // consumers of one must accept the other (see the pwsh-tool-and-executor
      // Agent Note).
      /* jscpd:ignore-start -- deliberate result-schema symmetry with oh-tool-bash. */
      schema: {
        oneOf: [
          {
            type: 'object',
    ...
    /* jscpd:ignore-start -- the execute path mirrors oh-tool-bash's by design (see the pwsh-tool-and-executor Agent Note). */
    async execute(args: PwshToolArgs, exec) {
      ...
      const request = {
        ...
        ohEnv: ctx.shellEnv.collect(exec),
      ...
      if (args.run_in_background === true) {
        ...
        if (jobs === undefined) {
          throw new Error('background jobs unavailable: load @open-harness/oh-jobs and @open-harness/oh-tool-jobs')
        }
