...
Every foreground and background model pwsh call receives a freshly collected trusted `OH_*` environment through the shared [`dsh-shell-env`](../shell-env/) registry: `OH_HOME` (the absolute Harness home), `OH_SHELL=1`, the agent's `OH_SESSION_ID`, and `OH_SESSION_JSONL` when the active persistence backend locates one. Plugins contributing `OH_*` facts to `ctx.shellEnv` apply to pwsh calls exactly as they do to bash calls. The snapshot passes through the dedicated `ShellExecRequest.dshEnv` channel; `process.env` is never modified. The description teaches the generic `$env:OH_*` convention rather than naming persistence-specific variables.
