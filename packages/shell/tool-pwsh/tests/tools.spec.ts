 ...
 * managed `OH_*` collection, abort translation, canonical result projection,
...
import type { ShellProcessRead } from '@open-harness/oh-shell'
...
async function setup(toolConfig: Partial<ToolPwsh.Config> = {}, ohHome?: string) {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
  await ctx.plugin(BashEnvPlugin, ohHome === undefined ? {} : { ohHome })
...
async function setupWithTasks(toolConfig: Partial<ToolPwsh.Config> = {}, ohHome?: string) {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
