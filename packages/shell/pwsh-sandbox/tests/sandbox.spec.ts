...
import type { ConfinedArgv, RunnerFailureRule, SandboxExecutionPolicy, SandboxPolicy } from '@open-harness/oh-sandbox'
import { resolvePwshPath } from '@open-harness/oh-pwsh-local'
...
const spillDir = mkdtempSync(join(tmpdir(), 'oh-pwsh-sandbox-spec-'))
...
describe('helpers (pure)', () => {
  const workdir = mkdtempSync(join(tmpdir(), 'oh-pwsh-sandbox-helpers-'))
  afterAll(() => {
...
describe.skipIf(!pwshAvailable())('SandboxPwshExecutor', () => {
  ...
  const readOnlyDir = mkdtempSync(join(tmpdir(), 'oh-pwsh-sandbox-ro-'))
