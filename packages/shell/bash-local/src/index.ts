...
import { installSettingsSection } from '@open-harness/oh-settings'
import { clampTimeout, deadline, MAX_TIMER_DELAY_MS, timeoutOf } from '@open-harness/oh-timeout'
...
export class LocalBashExecutor extends ShellExecutor {
  ...
  resolve(request: ShellExecRequest): ShellExecSpec {
    ...
    return {
      ...
      // Carry stdin/ordinary env/trusted ohEnv through verbatim — optional,
  ...
  private spawnSpec(
    ...
    return {
      ...
      signal,
      // One explicit env map for the seam, layered so the trusted ohEnv
      ...
      env: { ...ENV_OVERRIDES, ...spec.env, ...spec.ohEnv },
    }
  }
