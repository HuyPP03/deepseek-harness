 ...
 * `oh-acp-demo [--config path]`, defaulting to `./cordis.yml`. Shared env
 * loading, Loader guards, snapshot config selection, and settled-tree boot live
 * in oh-app-boot. Replay skips `.env` and selects sibling
 ...
 * @module @open-harness/oh-acp-demo/bin
 */

import { parseArgs } from 'node:util'
import { boot, installFailLoud, loadEnv, resolveConfigPath } from '@open-harness/oh-app-boot'

const NAME = 'oh-acp-demo'

/* v8 ignore start -- thin self-executing composition over the unit-tested
   oh-app-boot helpers; exercised end-to-end by the snapshot suite and the
   built-bin smoke */
