...
import { renderPrompt, TOOL_ORDER_REST } from '@open-harness/oh-system-prompt'
import * as agentCore from '../src/index.ts'
import { agentEvents, type Agent } from '@open-harness/oh-agent'
...
/**
 * Unit coverage for the @open-harness/oh-agent-spine-demo bundle: mounting it brings
...
async function mount(config: agentCore.Config, withBash = false): Promise<Context> {
  ...
  try {
    ...
    if (oldDshHome === undefined) {
      ...
      process.env.OH_HOME = oldDshHome
    }
    if (oldAgentsHome === undefined) {
      delete process.env.OH_AGENTS_HOME
    } else {
      process.env.OH_AGENTS_HOME = oldAgentsHome
    }
  }
}
...
describe('oh-agent-spine-demo bundle', () => {
  ...
  it('mounts package companions and forwards invariant selection config', async () => {
    ...
    for (const invariants of [
      { enabled: false },
      { package_allowlist: ['^@open-harness/oh-agent$'] },
      { package_blocklist: ['^@open-harness/oh-session$'] },
    ]) {
  ...
  it('loads workspace instructions into requests through the bundled spine', async () => {
    ...
    try {
      ...
      expect(adapter.requests[0]?.system).toContain('You are an AI agent powered by Open Harness.')
  ...
  it('shares top-level ohHome between local skills and the managed bash environment', async () => {
    ...
    expect(ctx.shellEnv.collect(execution)).toMatchObject({ OH_HOME: home, OH_SHELL: '1' })
    await ctx.fiber.dispose()
  })
