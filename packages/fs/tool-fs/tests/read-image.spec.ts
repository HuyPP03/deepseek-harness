...
import type { GenerateOptions, LlmModelInfo, LlmResolvedModelInfo, StreamChunk } from '@open-harness/oh-llm'
import SystemPrompt from '@open-harness/oh-system-prompt'
import ToolRuntime, { RUN_CODE_NAME } from '@open-harness/oh-tools'
import type { Config as ToolConfig } from '@open-harness/oh-tools'
import LocalFileSystem from '@open-harness/oh-fs-local'
import * as FsPolicy from '@open-harness/oh-fs-observation-policy'
import LocalAttachmentStore from '@open-harness/oh-attachment-local'
import { AttachmentId, AttachmentStore } from '@open-harness/oh-attachment'
...
async function setup(options: SetupOptions = {}) {
  ...
  if (options.attachments !== false) {
    await ctx.plugin(LocalAttachmentStore, { ohHome: home, ...options.storeConfig })
  }
...
describe('registration surface', () => {
  it('withdraws read_image when the tool-fs fiber or the attachment store is disposed (HMR safety)', async () => {
    ...
    await ctx.plugin(LocalFileSystem, { cwd: dir })
    await ctx.plugin(FsPolicy)
    const attachmentsFiber = await ctx.plugin(LocalAttachmentStore, { ohHome: home })
    ...
    const remounted = await ctx.plugin(LocalAttachmentStore, { ohHome: home })
    expect(names()).toEqual(['edit', 'read', 'read_image', 'write'])
