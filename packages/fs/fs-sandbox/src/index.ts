 ...
 * `@open-harness/oh-fs` Service Definition. It extends `LocalFileSystem` so all
 ...
 * stays `ctx.shell`'s job (`@open-harness/oh-bash-sandbox`). This mirrors the
 ...
 * escalation retry lives in the tool layer (`@open-harness/oh-tool-fs`),
...
import type { FsEditOutcome, FsEditRequest, FsTarget, FsVersion, FsWriteIntent, FsWriteOutcome } from '@open-harness/oh-fs'
...
import type {} from '@open-harness/oh-sandbox-policy'
 ...
 * INSTEAD OF `oh-fs-local`, together with a `ctx.sandboxPolicy`, is the whole
 * swap — the model-facing tools are untouched). Its configured default mode is
 * the capability fact exposed by {@link sandboxMode}; `oh-tool-fs` resolves
