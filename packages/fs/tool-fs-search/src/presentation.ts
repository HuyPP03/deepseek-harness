 ...
 * model-facing render consumes ({@link module:@open-harness/oh-tool-fs-search/search-core}
 ...
 * final output budget (`oh-spill-policy`) only shrinks `content`, never `meta`,
 * so this projection owns keeping `meta` bounded.
 *
 * @module @open-harness/oh-tool-fs-search/presentation
 */
 ...
 * ({@link module:@open-harness/oh-tool-fs-search/grep} `formatGrepMatches`), so
