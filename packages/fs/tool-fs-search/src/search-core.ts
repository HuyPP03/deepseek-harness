 ...
 *
 * @module @open-harness/oh-tool-fs-search/search-core
 */

import { isAbsolute, relative, sep } from 'node:path'
import type { Context } from '@open-harness/cordis'
import { HarnessError } from '@open-harness/oh-llm'
import { ItemRetainer, TextRetainer } from '@open-harness/oh-output-retention'
import type { RetainedItems } from '@open-harness/oh-output-retention'
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputRead, SubprocessSpawnSpec } from '@open-harness/oh-subprocess'
 ...
 * `@open-harness/oh-tool-call-timeout-policy` to enforce through `exec.signal`.
 */
 ...
 * (`oh-spill-policy`) only shrinks a result's `content`, never its `meta`, so the
 ...
 * (`@open-harness/oh-tool-call-timeout-policy`) and caller cancellation terminate the
 * process tree.
 *
 ...
 * retention pass both the model-facing render ({@link module:@open-harness/oh-tool-fs-search/grep}
 * `formatGrepOutput`) and the search-card projection
 * ({@link module:@open-harness/oh-tool-fs-search/presentation} `grepSearchMeta`)
