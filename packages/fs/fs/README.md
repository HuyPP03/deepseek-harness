...
| Layer | Package | Role |
|---|---|---|
| tool / executor | `@open-harness/oh-tool-fs` | model-facing `read`/`write`/`edit` schemas + read windowing + text rendering; reads/writes/edits via `ctx.fs`, dispatches the `fs/*` events |
| policy | `@open-harness/oh-fs-observation-policy` | observed-state + read-before-edit + version-guarded write/edit, contributed through the `fs/*` event gate (no service) |
| provider contract | `@open-harness/oh-fs` (this) | `ctx.fs`: execution-world paths, text IO, and atomic mutation primitives (optional version guard); owns the `fs/*` event vocabulary |
| provider | `@open-harness/oh-fs-local` | the host-filesystem implementation |
...
Indirectly, through `oh-tool-fs`, which renders provider text and errors as bounded, retained filesystem tool results.
