...
该包实现一个 skill（技能）来源。它扫描本地项目、自定义和用户 skill 根目录，解析 `SKILL.md` 或平铺 Markdown skill 文件，并将提供方注册到 `ctx.skills`。注册表仍位于 `@open-harness/oh-skill`；持久化会话目录和面向模型的 loader 工具仍位于 `@open-harness/oh-tool-skill`。
...
| `ohHome` | `$OH_HOME` 或 `~/.oh` | 由 [`@open-harness/oh-home-paths`](../../util/home-paths/README.md) 解析的 Open Harness 配置根目录；扫描该目录下的 `skills`。 |
| `agentsHome` | `$OH_AGENTS_HOME` 或 `~/.agents` | 为兼容 skill 扫描的共享 agent（智能体）配置根目录。 |
...
通过 `oh-tool-skill` 间接影响模型。它将该提供方的可调用名称和有长度上限的描述渲染到初始目录或替换目录中，并将所选的当前指令正文与资源基底指引渲染到保留的工具历史中；路径、提供方 rank 和已禁用 skill 仍被隐藏。
