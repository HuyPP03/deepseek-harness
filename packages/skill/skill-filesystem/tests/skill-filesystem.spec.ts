...
import { FileSystem, FsError, FsVersion, type FsDirEntry, type FsEditOutcome, type FsEditRequest, type FsInfo, type FsPathInfo, type FsTarget, type FsWriteOutcome } from '@open-harness/oh-fs'
...
describe('FileSystemSkillProvider', () => {
  it('discovers project, custom, user, and agents skill roots in priority order', async () => {
    ...
    await writeSkill(join(noGit, '.oh/skills'), 'fallback-root', 'Fallback root')
  ...
  it('lets project skills override runtime while runtime overrides custom and user skills', async () => {
    ...
    await writeSkill(join(project, '.oh/skills'), 'project-name', 'Project wins')
    await writeSkill(custom, 'runtime-name', 'Custom loses')
    await writeSkill(join(home, '.oh/skills'), 'runtime-name', 'User loses')
  ...
  it('parses flat skills and filters invalid skills from the invocation-neutral listing', async () => {
    ...
    const root = join(home, '.oh/skills')
  ...
  it('discovers symlinked skill directories and flat files', async () => {
    ...
    await mkdir(join(home, '.oh/skills'), { recursive: true })
    await symlink(join(external, 'linked-dir'), join(home, '.oh/skills/linked-dir'))
    await symlink(join(external, 'linked-flat.md'), join(home, '.oh/skills/linked-flat.md'))
    await symlink(join(external, 'missing'), join(home, '.oh/skills/broken-link'))
    await symlink('/dev/null', join(home, '.oh/skills/device-link'))
  ...
  it('forwards cancellation to filesystem reads while loading a skill', async () => {
    ...
    await writeSkill(join(home, '.oh/skills'), 'abortable-skill', 'Abortable skill')
  ...
  it('uses fs/observed as a synchronous first-party invalidation path without a watcher', async () => {
    ...
    emitObserved(join(home, '.oh/skills/.system/SKILL.md'), { name: 'write' })
  ...
  it('uses default home root resolution without exposing builtin skills', async () => {
    ...
    try {
      process.env.OH_HOME = join(envHome, '.oh')
      ...
      await writeSkill(join(envHome, '.oh/skills'), 'env-skill', 'Env skill')
