...
import { createUserMessage, CallId, type Message } from '@open-harness/oh-llm'
import { createScope, type Scope } from '@open-harness/oh-scope'
import { Session, SessionId, type SessionEvent, type UserMessage } from '@open-harness/oh-session'
...
import AgentRegistry, { agentEvents, Inbox, type Agent, type PreStepDecision } from '@open-harness/oh-agent'
...
describe('oh-tool-skill', () => {
  ...
  it('loads a skill for the calling agent cwd', async () => {
    ...
    await writeSkill(join(project, '.oh/skills'), 'project-skill', 'Project skill', 'Project instructions.')
    const ctx = await setup(home)
    ...
    expect(result.value).toEqual({
      name: 'project-skill',
      provider: 'filesystem',
      resourceBase: { kind: 'directory', path: join(project, '.oh/skills/project-skill') },
    ...
    expect(block.text).toBe([
      ...
      '<skill_resources>',
      `Base directory for this skill: ${join(project, '.oh/skills/project-skill')}`,
  ...
  it('returns isError for unknown, invalid, and model-disabled skills', async () => {
    ...
    await writeSkill(join(home, '.oh/skills'), 'hidden-skill', 'Hidden skill', 'Hidden instructions.')
    await writeFile(join(home, '.oh/skills/hidden-skill/SKILL.md'), '---\nname: hidden-skill\ndescription: Hidden skill\ndisable-model-invocation: true\n---\n\nHidden instructions.\n')
    const ctx = await setup(home)
    ctx.skills.register({
      name: 'model-only-skill',
      description: 'Model-only skill',
