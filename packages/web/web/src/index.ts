...
export class WebRuntime extends Service {
   ...
   * `$OH_WEB_SEARCH_PROVIDER` / `$OH_WEB_FETCH_PROVIDER` are equivalent to
  ...
  constructor(ctx: Context, config: WebRuntimeConfig = {}) {
    super(ctx, 'web')
    this.searchProviderId = config.searchProvider ?? process.env.OH_WEB_SEARCH_PROVIDER
    this.fetchProviderId = config.fetchProvider ?? process.env.OH_WEB_FETCH_PROVIDER
  }
