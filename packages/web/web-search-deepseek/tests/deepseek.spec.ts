...
} from '@open-harness/oh-web-search-deepseek'
import * as deepseekPlugin from '@open-harness/oh-web-search-deepseek'
import { citationSnippets, mapAnthropicResponse } from '../src/provider.ts'
import type { AnthropicResponse } from '@open-harness/oh-web-search-deepseek/src/types.ts'

/** Construct the provider over a fixed options value; production passes a live thunk. */
import type { DeepSeekSearchProviderOptions } from '@open-harness/oh-web-search-deepseek'
...
describe('web-search-deepseek plugin registration', () => {
  ...
  it('resolves the credential for each search so a stored or rotated key needs no restart', async () => {
    ...
    const dir = await mkdtemp(join(tmpdir(), 'oh-web-search-credentials-'))
