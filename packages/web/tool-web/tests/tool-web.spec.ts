...
import type { WebSearchProvider, WebSearchResult } from '@open-harness/oh-web'
...
} from '@open-harness/oh-tool-web'
import type { ContentBlock } from '@open-harness/oh-llm'
import type { ToolResult } from '@open-harness/oh-tools'
...
async function mountTools(opts: {
  ...
  fetchProvider?: import('@open-harness/oh-web').WebFetchProvider
...
describe('tool-web execution through the real registry', () => {
  ...
  it('executes web_fetch, forwarding the url (no timeout param) and the abort signal to the seam', async () => {
    ...
    // tool-call budget is owned by oh-tool-call-timeout-policy over exec.signal.
