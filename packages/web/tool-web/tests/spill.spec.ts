 ...
 * (`oh-spill-local` backend + `oh-spill-policy`), exercised through
...
import LocalSpillStore from '@open-harness/oh-spill-local'
import * as SpillPolicy from '@open-harness/oh-spill-policy'
...
beforeEach(async () => {
  ...
  spillRoot = mkdtempSync(join(tmpdir(), 'oh-spill-web-'))
