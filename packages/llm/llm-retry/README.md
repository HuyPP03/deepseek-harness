...
- name: '@open-harness/oh-llm-retry'
```

The executor has no policy config. Multi-provider adapters such as `oh-llm-pi-ai` place `retryPolicy` inside each provider profile, avoiding a second provider-name list.
