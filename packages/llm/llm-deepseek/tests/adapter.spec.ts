...
import { createLaunchEnvironmentSnapshot } from '@open-harness/oh-launch-environment'
...
import { getOrCreateAnonymousUserId, type AnonymousUserId } from '@open-harness/oh-anonymous-user-id'
import { SessionId } from '@open-harness/oh-session'
import * as LlmDeepSeek from '@open-harness/oh-llm-deepseek'
import { DeepSeekAdapter, resolveAdapterOptions } from '@open-harness/oh-llm-deepseek'
...
describe('plugin registration and config', () => {
  ...
  it('takes DEEPSEEK_BASE_URL from any environment layer, with explicit config still on top', () => {
    const trusted = createLaunchEnvironmentSnapshot([
      { source: 'user-env', path: '/home/.oh/.env', values: { DEEPSEEK_BASE_URL: 'https://user.example' } },
    ])
