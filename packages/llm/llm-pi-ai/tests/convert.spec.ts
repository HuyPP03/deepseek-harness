...
import { createUserMessage, CallId, CONTEXT_WINDOW_EXCEEDED_CODE, EMPTY_RESPONSE_CODE, createMessage } from '@open-harness/oh-llm'
import type { ContentBlock, StreamChunk } from '@open-harness/oh-llm'
...
describe('toPiContext', () => {
  ...
  it('degrades the flat pre-envelope replay state a legacy session log carries', () => {
    ...
    expect(context.messages[0]).toMatchObject({ role: 'assistant', api: 'oh-foreign' })
