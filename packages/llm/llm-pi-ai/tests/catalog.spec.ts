...
import LlmRuntime, { createUserMessage } from '@open-harness/oh-llm'
...
/** A throwaway $OH_HOME with an empty settings document. */
async function home(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'oh-pi-catalog-'))
  homes.push(dir)
