...
import { resolveRetryPolicy, RetryPolicySchema } from '@open-harness/oh-llm'
import type { ResolvedRetryPolicy, RetryPolicyConfig } from '@open-harness/oh-llm'
...
function rejectRemovedFields(provider: string, source: PiAiProviderProfile): void {
  ...
  if ('maxRetries' in legacy || 'maxRetryDelayMs' in legacy) {
    throw new Error(
      ...
      + ' compose agent recovery with oh-llm-retry',
    )
  }
}
