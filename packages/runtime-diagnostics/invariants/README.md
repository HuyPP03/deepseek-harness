...
Session itself owns immutable, surface-valid log storage in every composition: it takes one lossless JSON snapshot of each candidate, validates complete cited source-event coverage and positional replacement, restricts `tool/result` replacement to one current result's `content`, deep-freezes the accepted record, and exposes the log through immutable array snapshots. The `oh-session` invariant companion checks the remaining cross-record rules that Session does not own.
...
| Companion | Checks |
|---|---|
| `oh-session`, `oh-agent`, `oh-scope`, `oh-agent-loop` | Session enclosure and call/result trace, agent-status transitions, inbox FIFO conservation, scoped subjects, and model-request reconstruction. |
| `oh-llm`, `oh-llm-retry`, `oh-tools`, `oh-system-prompt` | Stream grammar, durable retry position and bounds, tool-pipeline stages and frozen results, and authoritative prompt-assembly data. |
| `oh-compaction`, `oh-hook-protocol`, `oh-sandbox-policy` | Durable compaction and hook pairing, compaction metadata, and sandbox-mode vocabulary. |
| `oh-fs`, `oh-subagent`, `oh-workflow` | Filesystem event identity, provider/child pairing, and workflow/agent lifecycle identity. |
| `oh-goal`, `oh-goal-round-driver` | Durable goal source/content agreement, revision and lifecycle transitions, timestamps, sequential admitted rounds, and reconstructed continuation prompts. |
| `oh-permission-presets`, `oh-user-approval` | Active-preset references and approval asked/decided audit pairing. |
| `oh-jobs`, `oh-tool-todo` | Task snapshot lifecycle/ownership fields and durable whole-list todo structure. |
| `oh-time-context` | Durable clock readings agree with the session's open turn and next pre-step position and elapsed baseline; rendered time parses and does not postdate its event. |
...
ctx.plugin(InvariantRegistry, {
  ...
  package_blocklist: ['^@open-harness/oh-agent-loop$'],
})
