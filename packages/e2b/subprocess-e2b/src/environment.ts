...
import { e2bControlEnvs } from '@open-harness/oh-e2b'
import type { Sandbox } from '@open-harness/oh-e2b'
import { SENSITIVE_ENV_PATTERN } from '@open-harness/oh-subprocess'
...
export async function readRemoteEnvironment(sandbox: Sandbox, signal?: AbortSignal): Promise<string> {
  ...
  const result = await sandbox.commands.run(
    'set -o pipefail; oh_e2b_passwd="$(getent passwd "$(id -u)")"; IFS=: read -r _ _ _ _ _ oh_e2b_home _ <<<"$oh_e2b_passwd"; test -n "$oh_e2b_home" -a -d "$oh_e2b_home"; printf \'%s\' "$oh_e2b_home" | base64 -w 0; printf \'\\n\'; env -0 | base64 -w 0',
...
export function scrubRemoteEnvironment(raw: string): Map<string, string> {
  ...
  for (const [name, value] of remoteEnvironmentEntries(raw)) {
    if (name.startsWith('OH_') || SENSITIVE_ENV_PATTERN.test(name)) continue
...
export function bootstrapEnvironment(raw: string): Record<string, string> {
  ...
  for (const [name] of remoteEnvironmentEntries(raw)) {
    if (name.startsWith('OH_') || SENSITIVE_ENV_PATTERN.test(name)) environment[name] = ''
  }
  return environment
}
