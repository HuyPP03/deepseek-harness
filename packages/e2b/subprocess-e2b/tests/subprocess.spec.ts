...
import type { SubprocessSpawnSpec } from '@open-harness/oh-subprocess'
import E2BSubprocessRuntime from '@open-harness/oh-subprocess-e2b'
...
describe('E2BSubprocessHandle', () => {
  it('starts asynchronously, keeps secrets out of the command, and supports deferred piped stdin/output', async () => {
    ...
    }), '/workspace/.oh-e2b/processes/one')
    ...
    const command = fake.commandsSeen.find(value => value.includes('exec "$oh_e2b_env_bin" -i'))!
    expect(command).toContain('"$oh_e2b_setsid" --wait -- "$oh_e2b_bash" -c')
    ...
    expect(command).not.toContain('${!oh_e2b_name}')
    ...
    expect(environmentProbe).toContain('test -n "$oh_e2b_home" -a -d "$oh_e2b_home"')
    expect(environmentProbe).not.toContain('"$PWD"')
    expect(command).toContain('mapfile -d')
    expect(command).toContain('oh_e2b_node="$(command -v node)"')
    expect(command).toContain('"$oh_e2b_env_bin" -i "$oh_e2b_node" -e')
    expect(command).toContain('"$oh_e2b_env_bin" -i -- "${oh_e2b_env[@]}" "$@"')
    expect(command).toContain('exec "$oh_e2b_env_bin" -i -- "${oh_e2b_env[@]}"')
    ...
    expect(fake.writtenFiles[0]).toEqual([
      '/workspace/.oh-e2b/processes/one/pid',
      '/workspace/.oh-e2b/processes/one/exit-code',
      '/workspace/.oh-e2b/processes/one/environment',
      '/workspace/.oh-e2b/processes/one/stderr.log',
    ])
    expect(fake.writtenFileData.get('/workspace/.oh-e2b/processes/one/environment')).toBe(
    ...
    expect(fake.removed).toContain('/workspace/.oh-e2b/processes/one/stderr.log')
  ...
  it('removes a spill once the complete stream exceeds its cap', async () => {
    ...
    const command = fake.commandsSeen.find(value => value.includes('oh_e2b_tee='))!
    expect(command).toContain('"$oh_e2b_head" -c 3')
    expect(command).toContain('/runtime/oversize/stdout.log')
    expect(command).toContain('"$oh_e2b_tee" --output-error=warn-nopipe')
