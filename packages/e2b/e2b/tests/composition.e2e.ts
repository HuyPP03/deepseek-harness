...
import { runLoaderSmoke } from '@open-harness/oh-loader-smoke'
import {
  FileNotFoundError,
  Sandbox,
  SandboxNotFoundError,
} from '@open-harness/oh-e2b'
import TerminalSessionService, { TerminalSessionId } from '@open-harness/oh-terminal'
...
describe.skipIf(!process.env.E2B_API_KEY)('E2B live Loader composition', () => {
  it('scrubs credentials before actual E2B command and PTY login shells', async () => {
    ...
    const sandbox = await Sandbox.create({
      apiKey,
      envs: { NPM_TOKEN: 'sentinel-secret', OH_STALE: 'sentinel-stale', KEEP: 'visible' },
      timeoutMs: 60_000,
      secure: true,
    ...
    try {
      ...
      const result = await session.startSend({
        text: "printf 'NPM=<%s> Open Harness=<%s> KEEP=<%s>\\n' \"$NPM_TOKEN\" \"$OH_STALE\" \"$KEEP\"",
        submit: true,
      }).done
      expect(result.viewport).toContain('NPM=<> Open Harness=<> KEEP=<visible>')
  ...
  it('runs FS, Bash, PTY, and LSP in one sandbox and deletes it', async () => {
    ...
    expect(terminalMotd).not.toContain('.oh-e2b/terminals/')
