...
function stageInstallation(bundles: Record<string, { patch?: string; deps?: Record<string, string> }>): string {
  ...
  for (const [name, spec] of Object.entries(bundles)) {
    ...
    writeFileSync(join(dir, 'package.json'), JSON.stringify({
      ...
      ...spec.patch === undefined ? {} : { oh: { bundle: { patch: './cordis.patch.yml' } } },
    }))
...
describe('initProfile', () => {
  it('creates manifest, user patch layer, and pnpm workspace once, never overwriting', () => {
    ...
    initProfile(dir, ['@open-harness/oh-base'])
    const manifest = readProfileManifest('t', dir)
    expect(manifest.oh?.profile?.bundles).toEqual(['@open-harness/oh-base'])
    expect(readFileSync(join(dir, PROFILE_PATCH_FILENAME), 'utf8')).toContain('[]')
    expect(readFileSync(join(dir, 'pnpm-workspace.yaml'), 'utf8')).toContain('nodeLinker: hoisted')
    // Re-init keeps user edits.
    writeFileSync(join(dir, PROFILE_PATCH_FILENAME), '- id: x\n  config: {}\n')
    initProfile(dir, ['other'])
    expect(readProfileManifest('t', dir).oh?.profile?.bundles).toEqual(['@open-harness/oh-base'])
...
describe('manifest round-trip', () => {
  it('writes and reads back, and fails loud on a broken manifest', () => {
    const dir = tmp()
    writeProfileManifest(dir, { name: 'p', oh: { profile: { bundles: ['a'] } } })
    expect(readProfileManifest('t', dir).oh?.profile?.bundles).toEqual(['a'])
...
describe('loadProfile', () => {
  it('resolves each oh.profile.bundles entry to its patch layer in order, plus the user layer', () => {
    const anchor = stageInstallation({
  ...
  it('auto-initializes only shipped templates and fails loud otherwise', () => {
    ...
    // @open-harness/* through tsconfig paths regardless of the staged anchor.
    expect(PROFILE_TEMPLATES.web).toContain('@open-harness/oh-base')
    try {
      loadProfile('t', 'web', anchor, home)
    } catch {
      // Resolution failure is the plain-Node outcome for this empty anchor.
    }
    expect(readProfileManifest('t', resolveProfileDir('web', home)).oh?.profile?.bundles)
      .toEqual([...PROFILE_TEMPLATES.web ?? []])
  })

  it('normalizes only the exact installation-owned headless bundle tuple', () => {
    const anchor = stageInstallation({
      '@open-harness/oh-base': { patch: '[]\n' },
      '@open-harness/oh-web-app': { patch: '[]\n' },
      '@open-harness/oh-headless': { patch: '[]\n' },
      'custom-bundle': { patch: '[]\n' },
    })
    const home = tmp()
    const stock = resolveProfileDir('headless', home)
    initProfile(stock, [
      '@open-harness/oh-base', '@open-harness/oh-web-app', '@open-harness/oh-headless',
    ])
    loadProfile('t', 'headless', anchor, home)
    expect(readProfileManifest('t', stock).oh?.profile?.bundles)
      .toEqual(['@open-harness/oh-base', '@open-harness/oh-headless'])

    const customHome = tmp()
    const custom = resolveProfileDir('headless', customHome)
    initProfile(custom, [
      '@open-harness/oh-base', '@open-harness/oh-web-app', '@open-harness/oh-headless', 'custom-bundle',
    ])
    loadProfile('t', 'headless', anchor, customHome)
    expect(readProfileManifest('t', custom).oh?.profile?.bundles).toEqual([
      '@open-harness/oh-base', '@open-harness/oh-web-app', '@open-harness/oh-headless', 'custom-bundle',
    ])
  })

  it('fails loud when a listed bundle declares no oh.bundle', () => {
    const anchor = stageInstallation({ 'not-a-bundle': {} })
    const home = tmp()
    const dir = resolveProfileDir('demo', home)
    initProfile(dir, ['not-a-bundle'])
    expect(() => loadProfile('t', 'demo', anchor, home)).toThrow('declares no oh.bundle')
  })
})
