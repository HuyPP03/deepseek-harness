 ...
 * @module @open-harness/oh-app-boot
 */
...
import { Context, type FiberState } from '@open-harness/cordis'
import Loader, { type Entry, type EntryOptions } from '@open-harness/cordis-plugin-loader'
import Include, { applyEntryPatches, entryListSchema, type PatchOptions } from '@open-harness/cordis-plugin-include'
import Group from '@open-harness/cordis-plugin-group'
import { ohHomePath, resolveOhHome } from '@open-harness/oh-home-paths'
import { createLaunchEnvironmentSnapshot, type LaunchEnvironmentSnapshot } from '@open-harness/oh-launch-environment'
import type {} from '@open-harness/cordis-plugin-hmr'
...
export {
  ...
  type OhBundleManifest,
  type OhManifestSection,
  type OhProfileManifest,
  type Profile,
  type ProfileLayer,
 ...
 * @param snapshotMode - the bin's `$OH_SNAPSHOT` value; only `'replay'` swaps the
 *   basename.
...
const BOOTSTRAP_PREFIXES = ['OH_', 'XDG_', 'DYLD_', 'BASH_FUNC_']
 ...
 * entries (`@open-harness/cordis-plugin-include`'s `PatchOptions`): id-targeted config
 ...
 * `@open-harness/cordis-plugin-include` `PatchOptions` (id-targeted config overrides and
...
export async function mountRootInclude(
  ...
  // preset living outside this workspace cannot resolve `@open-harness/cordis-plugin-group`
 ...
 * directory. The self-referential `oh-tool-cordis` toolset reads and edits this
...
export function addHarnessSourceSection(ctx: Context, sourceRoot: string): (() => void) | undefined {
  ...
  return systemPrompt.section({
    ...
    order: -99,
    text: `The Open Harness implementation checkout is at ${sourceRoot}. The checkout location and current working directory are separate values and may differ; never infer the working directory from this path. Use pwd to determine the current working directory. Use this checkout only to inspect or extend Open Harness itself.`,
  })
}
