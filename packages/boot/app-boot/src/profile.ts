 ...
 * flat fallback directory `$OH_HOME/profiles/node_modules` (one symlink per
 ...
 * @module @open-harness/oh-app-boot/profile
 */
...
export const PROFILE_TEMPLATES: Record<string, readonly string[]> = {
  web: ['@open-harness/oh-base', '@open-harness/oh-web-app'],
  headless: ['@open-harness/oh-base', '@open-harness/oh-headless'],
}

/** Installation-owned bundle tuples normalized to the shipped template. */
const INSTALLATION_OWNED_PROFILE_TUPLES: Record<string, readonly string[]> = {
  headless: ['@open-harness/oh-base', '@open-harness/oh-web-app', '@open-harness/oh-headless'],
}

/** The bundle list an `oh plugin` init uses for a name with no shipped template. */
export const DEFAULT_PROFILE_BUNDLES: readonly string[] = ['@open-harness/oh-base']
...
/**
 * Maintain the flat module fallback `$OH_HOME/profiles/node_modules`: one
 ...
 * Definition packages (`oh-compaction`, `oh-invariants`, ...) that the app
...
export function healProfilesModuleFallback(installAnchor: string, home: string = resolveOhHome()): void {
  ...
  for (let next = queue.shift(); next !== undefined; next = queue.shift()) {
    // Peer dependencies participate: Service Definition packages (oh-subprocess,
    // oh-compaction, ...) are peers of their implementations, never plain
 ...
 * `@open-harness/oh-base` (and every other in-box bundle) always comes from
