...
- **The `/mode` command.** The service registers `mode` on the command registry when one is composed: `/mode` reports the current preset and roster, and `/mode <preset>` switches the session's composition mid-conversation. The guards: the agent must be IDLE — a running turn owns its composition for the duration of its steps, and the switch re-reads that state under a per-session lock — and a preset configured in `chatPresetIds` is never crossed in either direction, because a chat session is a fixed conversation surface, not a mode.
