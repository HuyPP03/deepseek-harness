 ...
 * @module @open-harness/oh-agent-presets/mount
 */
...
import type { EntryTree } from '@open-harness/cordis-plugin-loader'
import { scopeOf, scopeParentOf, type ScopeKey } from '@open-harness/oh-scope'
...
class PresetTree extends Include {
   ...
   * dependencies, so every `@open-harness/oh-*` row would fail to import. The
 ...
 * invariant companion's service listener, and `oh-invariants` is a
