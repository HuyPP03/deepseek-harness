...
import type {} from '@open-harness/oh-cordis-host-runner'
...
export function describeDynamic(ctx: Context, agent?: Agent): string[] {
  ...
  if (rows.length === 0) {
    return ['No dynamic Plugins are defined in this session. Definitions live only in this process\'s memory, so an Open Harness restart clears them.']
  }
