...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    occupants: [
      'client-ui-file-inspector FileInspector',
    ],
    replaceRisk: 'shadows-shipped-ui',
  ...
  {
    key: 'conversation.details.files',
    kind: 'single',
    scope: 'session',
    summary: 'The body of the details panel for a session file browse — one occupant that renders the session\'s working-set file list (the bounded walk the composer\'s `@` source shares) so any file the session can address — including one created outside the mutation tools — gets an inspector entry point.',
    doc: 'The body of the details panel for a session file browse — one occupant\nthat renders the session\'s working-set file list (the bounded walk the\ncomposer\'s `@` source shares) so any file the session can address —\nincluding one created outside the mutation tools — gets an inspector\nentry point. The owner passes the browsed directory (the session cwd)\nand the display-only workspace root.',
    registerOptions: [],
    ownerProps: [
    ...
    declaredBy: 'an entry in \'details\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'client-ui-file-inspector FileBrowser',
    ],
    replaceRisk: 'shadows-shipped-ui',
  ...
  {
    key: 'conversation.details.job',
    kind: 'single',
    scope: 'session',
    summary: 'The body of the details panel for the background job the user selected — one occupant that renders the job\'s retained log (a live tail while the job runs, the settled output once terminal).',
    doc: 'The body of the details panel for the background job the user selected —\none occupant that renders the job\'s retained log (a live tail while the\njob runs, the settled output once terminal). The owner passes the\nregistry-issued job id; the occupant reads the log itself (the shared\nconnection\'s `jobs.log` unary) and owns its own polling, status line,\nand error presentation.',
    registerOptions: [],
    ownerProps: [
    ...
    declaredBy: 'an entry in \'details\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'client-ui-jobs JobDetailPanel',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'conversation.details.job\', () => ctx.slots.register(\n      { name: \'conversation.details.job\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-file-inspector FilesAction id \'session-files\'',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'root\' (client-ui-layout), so it exists while that entry is mounted',
    occupants: [
      'client-ui-connectors ConnectorsDirectory',
    ],
    replaceRisk: 'shadows-shipped-ui',
  ...
  {
    key: 'sidebar.connectors',
    kind: 'single',
    scope: 'root',
    summary: 'The predefined external connections browsing region, shown when the shell\'s active tab is \'connectors\'.',
    doc: 'The predefined external connections browsing region, shown when the\nshell\'s active tab is \'connectors\'. Declared by this package\'s \'sidebar\'\nentry; ui-connectors registers the roster region.',
    registerOptions: [],
    ownerProps: [
    ...
    occupants: [
      'client-ui-connectors ConnectedProvidersList',
    ],
    replaceRisk: 'shadows-shipped-ui',
