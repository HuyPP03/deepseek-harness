...
import type { SessionEvent, SessionId } from '@open-harness/oh-session'
import type { TypertContext, TypertLookup } from '@open-harness/oh-typert-protocol'
...
declare module '@deepseek-ai/cordis' {
  interface Context {
     ...
     * `oh-scope` tag while retaining it, so this field is DX context rather
...
export interface CreateAgentOptions {
   ...
   * {@link CreateSessionOptions.meta} in oh-session (the internal-only
 ...
 * via {@link AgentRegistry.setFactory}. Kept on the `oh-agent` interface so
 * consumers (e.g. the ACP bridge) program against `ctx.agents` without
 * depending on the concrete `oh-agent-loop` package.
 */
 ...
 * (`@open-harness/oh-agent-loop`), registered via {@link setFactory}.
 *
  ...
  constructor(ctx: Context) {
    ...
    ctx.inject(['typert'], (typeCtx) => {
      typeCtx.typert.lookups.register('agent', {
        parameter: 'agent',
        wire: 'agentId',
        hostTypeSymbol: '@open-harness/oh-agent#Agent',
        wireTypeSymbol: '@open-harness/oh-session/types#SessionId',
