...
import { assertNever, deepFreeze, HarnessError } from '@open-harness/oh-llm'
import type { Agent } from '@open-harness/oh-agent'
import { snapshotJsonValue } from '@open-harness/oh-session'
import type { JsonValue, UserMessage } from '@open-harness/oh-session'
import type { ToolProviderResult } from '@open-harness/oh-system-prompt'
import type { CodeRuntime } from '@open-harness/oh-code-runtime'
 ...
 * instead of deriving them: the seam's `oh-code-runtime` README pair, its
...
export type { JsonValue } from '@open-harness/oh-session'
...
// lives in its own UI-facing module; re-export it so `@open-harness/oh-tools`
...
declare module '@deepseek-ai/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@open-harness/oh-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@open-harness/oh-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@open-harness/oh-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@open-harness/oh-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@open-harness/oh-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  /** Internal staged view consumed by `oh-agent-loop`'s parallel scheduler. */
  ...
  private sdkSection(): { name: string; order: number; text: (context: { scope?: ScopeKey }) => string } {
    return {
      ...
      text: (context) => {
        ...
        if (render === undefined) throw new Error(`oh-tools: no SDK renderer for ${runtime.language}`)
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`oh-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @open-harness/oh-code-runtime-worker-thread) or set tools mode to "native"`)
    }
    if (!Object.hasOwn(SDK_RENDERERS, runtime.language)) {
      const known = Object.keys(SDK_RENDERERS).map(name => JSON.stringify(name)).join(', ')
      throw new Error(`oh-tools: no SDK renderer registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
    }
    return runtime
  }
   ...
   * `oh-agent-tool-presentation` exists for, and reading the deployment default would
