 ...
 * @module @open-harness/oh-tools/src/code-mode
 */

import { CallId, createUserMessage, HarnessError } from '@open-harness/oh-llm'
import type { ContentBlock } from '@open-harness/oh-llm'
import type { CodeBindingFunction, CodeRunResult, CodeRuntime } from '@open-harness/oh-code-runtime'
...
function resolveFlavor(peekRuntime: () => CodeRuntime | undefined): RunCodeFlavor {
  ...
  if (!Object.hasOwn(RUN_CODE_FLAVORS, runtime.language) || flavor === undefined) {
    ...
    throw new Error(`oh-tools: no run_code schema flavor registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
  }
  return flavor
}
