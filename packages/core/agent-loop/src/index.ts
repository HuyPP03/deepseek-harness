...
import { SessionId, SessionPreparation } from '@open-harness/oh-session'
import type { Session, SessionHeader } from '@open-harness/oh-session'
...
export class AgentLoop extends Service implements AgentFactory {
  ...
  async resume(ownerCtx: Context, options: ResumeAgentOptions): Promise<AgentHandle> {
    ...
    if (persistence === undefined) {
      throw new Error('cannot resume: session persistence is not configured (load a oh-session-persistence backend)')
    }
