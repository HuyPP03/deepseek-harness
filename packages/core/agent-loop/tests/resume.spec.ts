...
import SessionStore, { SESSION_FORMAT_VERSION, Session, SessionId, SessionPreparation } from '@open-harness/oh-session'
...
async function persistentHarness(adapter: MockAdapter): Promise<{ ctx: Context; root: string }> {
  const root = await mkdtemp(join(tmpdir(), 'oh-resume-'))
  dirs.push(root)
