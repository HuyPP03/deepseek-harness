...
import { normalizeTerminalText, TerminalSanitizer } from '@open-harness/oh-terminal-bash/src/sanitize.ts'
...
describe('TerminalSanitizer', () => {
  it('removes split CSI and owned OSC prompt markers', () => {
    ...
    expect(sanitizer.push('D;0\x07oh> ')).toEqual({ text: 'oh> ', prompt: true, promptTail: 'oh> ' })
  })
  ...
  it('reports printable prompt text that follows a marker in a later chunk', () => {
    ...
    expect(sanitizer.push('oh> ')).toEqual({ text: 'oh> ', prompt: false, promptTail: 'oh> ' })
  })
