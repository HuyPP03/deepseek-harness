...
import type { TerminalSendOperation } from '@open-harness/oh-terminal'
...
describe('terminal-bash real shell', () => {
  it('persists cwd and environment across sends, scrubs secrets, and closes', async () => {
    const previous = process.env.OH_TEST_SECRET
    process.env.OH_TEST_SECRET = 'must-not-leak'
    try {
      ...
      if (previous === undefined) delete process.env.OH_TEST_SECRET
      else process.env.OH_TEST_SECRET = previous
    }
  }, 10_000)
