...
The six generated schemas are listed in the [`oh-tool-terminal` catalog section](../../../docs/tool-catalog.md#open-harnessoh-tool-terminal). Their fixed schema tokens are present whenever this plugin is active; agent-scoped tool filtering may hide them.
...
- Background mode requires both `@open-harness/oh-jobs` and its model-facing controller.
