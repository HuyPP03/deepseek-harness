...
import type { SubagentCapabilities, SubagentProvider, SubagentResult, SubagentRun, SubagentStartRequest } from '@open-harness/oh-subagent'
import type { WorkflowMeta, WorkflowResult, WorkflowResultInfo, WorkflowRun, WorkflowRunInfo } from '@open-harness/oh-workflow'
...
describe('oh-workflow-worker-thread', () => {
  describe('script execution over a real worker thread', () => {
    ...
    it('the unbuilt worker forwards exactly TSX_TSCONFIG_PATH through the scrub: the paths-map pin survives, secrets do not', async () => {
      ...
      // must inherit the pin (or its oh-* imports silently resolve to
