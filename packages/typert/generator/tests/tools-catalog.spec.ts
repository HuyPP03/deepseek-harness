...
import type { TypertContribution } from '@open-harness/oh-typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@open-harness/oh-tool-cordis/src/api-catalog.ts'
...
describe('model-driven oh-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 30_000 }, async () => {
    const workspace = new WorkspaceAnalyzer({
      root: workspaceRoot,
      faces: ['host'],
      packages: ['@open-harness/oh-tools'],
    }).analyze()
    const host = workspace.faces.find(candidate => candidate.face === 'host')
    if (host === undefined) throw new Error('oh-tools has no analyzed host face')
    const artifact = new FaceModelEmitter(host).emit('@open-harness/oh-tools')
    ...
    const record = ctx.typert.getPackage('@open-harness/oh-tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@open-harness/oh-tools', 'host')).toBeUndefined()
  })
})
