...
} from '@open-harness/oh-typert-registry'
import type {
...
declare module '@open-harness/oh-typert-protocol' {
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@open-harness/oh-tools', 'ToolInput')).toBe('@open-harness/oh-tools#ToolInput')
    expect(typertPackageKey('@open-harness/oh-tools', 'host')).toBe('@open-harness/oh-tools#host')
    expect(ctx.typert.get('@open-harness/oh-tools#ToolInput')).toMatchObject({
      package: '@open-harness/oh-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@open-harness/oh-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@open-harness/oh-tools', 'host')).toMatchObject({
      key: '@open-harness/oh-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@open-harness/oh-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@open-harness/oh-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@open-harness/oh-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@open-harness/oh-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@open-harness/oh-tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@open-harness/oh-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@open-harness/oh-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@open-harness/oh-tools#ToolInput')).not.toBe(projected)
  })
