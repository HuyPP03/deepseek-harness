...
import * as typertLoader from '@open-harness/oh-typert-loader'
import { validateTypertManifest } from '@open-harness/oh-typert-loader'
import { z } from 'zod'
...
describe('typert loader', () => {
  it('registers an explicit package without a Loader entry and withdraws it with the loader', LOADER_TEST_TIMEOUT, async () => {
    root = await mkdtemp(join(tmpdir(), 'oh-typert-loader-'))
    await linkZod(root)
