...
- **No cross-home identity** — different `$OH_HOME` values cannot be correlated.
- **Configured DeepSeek gateways receive the id** — `oh-llm-deepseek` sends the stable header to its resolved `baseURL`, including deployment overrides, independently of telemetry sharing mode.
