...
import SessionPersistence, { SessionPersistenceRevision } from '@open-harness/oh-session-persistence'
import type { SessionPersistenceSnapshot } from '@open-harness/oh-session-persistence'
import SqliteSessionPersistence from '@open-harness/oh-session-persistence-sqlite'
import SqliteSessionQueryEngine, {
  SESSION_QUERY_SQLITE_SCHEMA_VERSION,
} from '@open-harness/oh-session-query-sqlite'
import {
...
} from '@open-harness/oh-session-query'
...
async function temporaryPath(name = 'search.db'): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), 'oh-session-search-'))
