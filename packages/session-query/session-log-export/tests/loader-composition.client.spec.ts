...
import * as SessionLogDownload from '@open-harness/oh-session-log-export'
...
describe('session-log-download real Loader composition', () => {
  it('discovers and executes /export through the assembled command plane', async () => {
    root = await mkdtemp(join(tmpdir(), 'oh-session-export-loader-'))
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@open-harness/oh-session-log-export', SessionLogDownload],
    ])
