...
import { JobId, JobRegistry } from '@open-harness/oh-jobs'
import type {
  JobDoneListener, JobLogRead, JobRead, JobSnapshot, JobStart, JobsChangedListener,
 ...
 * behavior suite lives with `@open-harness/oh-jobs-local`.
 */
...
describe('JobRegistry seam', () => {
  ...
  it('mounting the abstract seam directly fails loudly at load (stale-composition fence)', async () => {
    ...
    await expect(ctx.plugin(JobRegistry as unknown as typeof StubJobRegistry))
      .rejects.toThrow(/abstract job registry seam; load an implementation such as @open-harness\/oh-jobs-local/)
  })
})
