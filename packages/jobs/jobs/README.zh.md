# @open-harness/oh-jobs

[English](README.md) | 中文

后台任务注册表约定（`ctx.jobs`）。抽象的 `JobRegistry` 及其词汇类型在同一份约定下为长时间运行的生产方提供共享 id、owner 隔离、读取、取消、等待、通知和清理；进程局部注册表位于 [`oh-jobs-local`](../jobs-local/README.md)。生产方插件使用其不透明 id namespace 扩展 `JobKindMap`。
...
- `log(id, caller?)` 是面向人类界面的读取（浏览器的任务详情面板）：返回保留的输出，既不推进 `read` 的游标，也不把任务标记为已报告，因此模型的下一次读取依然完整。流任务返回保留的尾部（受实现的保留上限约束）；最终输出任务在存活期间返回空，结算后返回终止输出。
...
通过生产方插件和 [`oh-tool-jobs`](../tool-jobs/README.md) 间接影响；它们会渲染 job id、输出、状态、取消和完成通知。
...
- **流输出只有一个消费游标**：`log()` 提供非消费式的人类读取尾部，但其他独立观察者仍然需要游标或快照 API。
