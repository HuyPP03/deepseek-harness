/**
 * End-to-end tests for oh-mcp-client. Exercises the REAL MCP protocol against:
...
import { publicToolName } from '@open-harness/oh-mcp-client/src/tools.ts'
...
async function mountImageRegistry(ohHome: string): Promise<Context> {
  const ctx = await mountRegistry()
  await ctx.plugin(LocalAttachmentStore, { ohHome })
