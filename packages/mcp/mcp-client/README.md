...
- Registry report: when the [`mcpRegistry`](../mcp-registry/README.md) service is mounted, the instance reports its server (`serverName`, lifecycle status `connecting`/`connected`/`reconnecting`/`down`, and the registered tool snapshot) so `/mcp` can list it; plugin disposal drops the entry. Without the service the bridge is unchanged.
