 ...
 * zero-build dev path, resolving `@open-harness/oh-*` / `@cordisjs/*` through the tsconfig `paths`
 ...
 *
 * @module @open-harness/oh-loader-smoke
 */
...
export const EXAMPLE_MODE_ENV = 'OH_EXAMPLE_MODE'
 ...
 * @param raw - the raw value; defaults to `process.env.OH_EXAMPLE_MODE`.
...
export interface LoaderSmokeOptions {
  ...
  readonly mode?: ExampleMode
  /** Environment overrides layered over the parent and isolated Open Harness homes. */
...
export async function runLoaderSmoke(options: LoaderSmokeOptions): Promise<LoaderSmokeResult> {
  ...
  try {
    ...
    const launch = resolveExampleLaunch({
      ...
      env: { OH_HOME: join(cwd, '.oh'), OH_AGENTS_HOME: join(cwd, '.agents'), ...options.env },
    })
