...
interface ScriptedLog {
  /** Path relative to `$OH_SNAPSHOT_SESSIONS_ROOT`, e.g. `project/session/session.jsonl`. */
  file: string
  /**
...
interface Behavior {
  ...
  /** Echo the `OH_SNAPSHOT_*` env the harness set as a chunk (spec-side env-plumbing assertions). */
  echoEnv?: boolean
...
const sessionsRoot = process.env.OH_SNAPSHOT_SESSIONS_ROOT ?? ''
const fixtureFile = process.env.OH_SNAPSHOT_FILE ?? ''
...
async function handlePrompt(id: number | string): Promise<void> {
  ...
  if (behavior.echoEnv === true) {
    chunk(`env:${JSON.stringify({
      mode: process.env.OH_SNAPSHOT,
      override: process.env.OH_SNAPSHOT_OVERRIDE ?? null,
      childFiles: process.env.OH_SNAPSHOT_CHILD_FILES ?? null,
      spillRoot: process.env.OH_SNAPSHOT_SPILL_ROOT ?? null,
      // Scenario-supplied deployment env (the `Scenario.env` layering hook).
      permissionMode: process.env.OH_PERMISSION_MODE ?? null,
    })}`)
  }
