...
export class FixtureSession implements SessionFace {
  ...
  /**
   * Fail-loud stub; supply `setReferences` on the fixture's session face to exercise it.
   * @returns never — always throws.
   */
  setReferences(): never {
    throw new Error(`test session "${this.sessionId}": setReferences is not stubbed — supply it on the fixture's session face`)
  }
}
