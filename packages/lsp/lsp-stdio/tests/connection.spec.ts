...
import { LspConnection } from '@open-harness/oh-lsp-stdio'
import type { ConnectionWriter } from '@open-harness/oh-lsp-stdio/src/connection.ts'
import { scrubbedParentEnv } from '@open-harness/oh-subprocess'
import { spawnSubprocess } from '@open-harness/oh-subprocess-local/src/spawn.ts'
...
describe('LspConnection', () => {
  ...
  it('forwards explicit OH_* env entries to the child', async () => {
    // A configured OH_* fact must reach the child: the seam scrubs only the
    // ambient namespace, and the explicit entry merges after that scrub. The
    // fixture echoes the named variable back as hover text.
    const conn = connect({ LSP_FAKE_ECHO_ENV: 'OH_LSP_TEST_FACT', OH_LSP_TEST_FACT: 'managed' })
