...
/**
 * Keyless built-artifact smoke: plain Node imports `@open-harness/oh-lsp` and
 * `@open-harness/oh-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      ...
      const { default: Lsp } = await import('@open-harness/oh-lsp')
      const LspLocal = await import('@open-harness/oh-lsp-stdio')
      const { default: LocalFileSystem } = await import('@open-harness/oh-fs-local')
      const { default: LocalSubprocessRuntime } = await import('@open-harness/oh-subprocess-local')
