...
- **详情面板是只读投影**：可从聊天中的工具调用行或产物文件行（`openDetails`）打开，也可从会话标题栏的“会话文件”动作（浏览模式）打开，后者经由本包提供的 `detailsPanel` 服务跨插件抵达——这是认可的跨插件手势通道。工具席位没有 Input/Output/Metadata 切换、Prev/Next 步进，也没有 trajectory 深链接。
