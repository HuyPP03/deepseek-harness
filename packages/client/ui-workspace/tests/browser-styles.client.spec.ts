...
describe('WorkspaceBrowser.module.css list', () => {
  ...
  it('counts the themed scrollbar inside the shell trailing inset', () => {
    expect(root?.get('--oh-session-list-edge-inset')).toBe('var(--oh-sidebar-inline-padding)')
    expect(root?.get('--oh-session-list-scrollbar-width')).toBe('8px')
    expect(root?.get('--oh-session-list-scrollbar-offset')).toBe('2px')
    expect(root?.get('padding-right')).toBe('var(--oh-session-list-edge-inset)')
    expect(listArea?.get('margin-left')).toBe('-4px')
    expect(listArea?.get('padding-left')).toBe('4px')
    expect(listArea?.get('margin-right')).toBe('calc(-1 * var(--oh-session-list-edge-inset))')
    expect(declarations('.fade')?.get('right')).toBe('var(--oh-session-list-edge-inset)')
    expect(list?.get('margin-right')).toBe('var(--oh-session-list-scrollbar-offset)')
    expect(list?.get('margin-left')).toBe('-4px')
    expect(list?.get('padding-left')).toBe('4px')
    expect(list?.get('padding-right')).toBe([
      'calc(',
      'var(--oh-session-list-edge-inset)',
      '- var(--oh-session-list-scrollbar-width)',
      '- var(--oh-session-list-scrollbar-offset)',
      ')',
    ].join(' '))
  ...
  it('keeps the compact fade, overflow control, search field, and row heights', () => {
    ...
    expect(rowDeclarations('.sessionRow.selected')?.get('background'))
      .toBe('var(--oh-alias-interactive-bg-hover)')
  })
