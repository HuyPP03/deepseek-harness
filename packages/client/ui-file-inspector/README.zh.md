...
对话文件检视器：`conversation.details.file` 槽位的占用方（该槽位由 `oh-client-ui-conversation` 的 details 入口声明），当用户选中一个文件时打开——来自 diff 卡片行、read 行或产物文件条目。一个面板，选中文件至多三个座位：
...
第二个座位覆盖没有被变更工具宣布过的文件：会话文件列表。会话标题栏的**会话文件**操作（header-actions 的占用方，仅当会话记录了 cwd 时渲染）在 `conversation.details.files` 座位上打开 details 面板，该座位通过 `files.list` RPC 列出会话的工作集（与 composer 的 `@` 来源共用的有界遍历：深度 8、扫描 20000 项、100 行，点开头项与跳过表在 host 侧剪枝）。目录行是不可交互的上下文；点击文件行会重新选中该文件，检视器常规座位（含预览）从同一面板接管。
...
- **文件列表是每次打开时的一份有界快照** — `files.list` 遍只在座位挂载时 fetch 一次（重新打开面板会重新 fetch；列表打开期间没有实时刷新），展示其 100 行的窗口并附截断提示，过滤是对该窗口的本地过滤而非 host 侧查询。
- **文件列表没有按目录导航** — 它平铺展示会话各根的相对路径；选中子目录不是单独的浏览步骤。
