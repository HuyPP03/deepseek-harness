...
import type { RunningToolCall, ToolResultNode } from '@open-harness/oh-client-runtime/client'
...
function settled(over: Partial<ToolResultNode> = {}): ToolResultNode {
  return {
    ...
    time: 3_000,
    callId: 'call-skill',
    call: { name: 'skill', argsRaw: '{"name":"oh-manage-issues"}' },
    callTime: 2_000,
...
function running(argsRaw = '{"name":"oh-manage-issues"}'): RunningToolCall {
  return {
...
describe('SkillRow', () => {
  it('renders a compact Bash-shaped summary and discloses the exact instructions', () => {
    ...
    const row = screen.getByRole('button', { name: 'Skilloh-manage-issues' })
    ...
    expect(view.container.textContent).not.toContain('{"name":"oh-manage-issues"}')
  ...
  it('keeps a running call compact and announces its state', () => {
    ...
    expect(view.container.textContent).toContain('oh-manage-issues')
    expect(view.container.querySelector('svg[stroke="currentColor"]')).not.toBeNull()
  })
