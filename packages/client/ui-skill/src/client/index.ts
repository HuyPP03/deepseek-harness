 ...
 * determinism
 * lives host-side — the pre-step boundary (`oh-tool-skill`) recognizes a
...
export function apply(ctx: ClientContext): void {
  ctx.effect(() => ctx.locale.register(NS, { en, vi, zh }), 'ui-skill: dictionaries')
  ...
  const source: InputTriggerSource = {
    ...
    onPick({ candidate }) {
      ...
      // pre-step boundary (oh-tool-skill) recognizes the leading /name and
