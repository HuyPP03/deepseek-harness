...
`SidebarRootComponentProps` 组合布局 owner share、全局 `useSessions` 和 `useWorkspaces` 钩子、已声明的 `sidebar.workspaces`、`sidebar.connectors` 与 `sidebar.settings` 子 slot、外壳标签 store，以及注入的 `startSession`／`startChat` 与侧边栏切换回调。
