...
The shell owns the browsing tab store (`chats` | `workspaces` | `connectors`, persisted as `dsh.sidebar.view.v2`, default `chats`): the wide column renders the tab list above the New button, and the active tab selects the region registrant — `chats` and `workspaces` hand the tab to the `sidebar.workspaces` owner, which renders the flat chat list or the Workspace tree, while `connectors` swaps the region for the `sidebar.connectors` registrant (the [ui-connectors](../ui-connectors/README.md) roster). The wordmark and New button act on the active tab: Workspaces starts the runtime's page-local frontend Session Intent; the other tabs start a chat, reusing the current blank chat session or minting one with `agentPreset: 'chat'` and opening it. The rail has no tab control; its New icon follows the persisted tab.
...
`SidebarRootComponentProps` composes the layout owner share, the global `useSessions` and `useWorkspaces` hooks, the declared `sidebar.workspaces`, `sidebar.connectors`, and `sidebar.settings` child slots, the shell tab store, and injected `startSession`/`startChat` plus sidebar-toggle callbacks.
