...
The shell owns the browsing tab store (`chats` | `workspaces`, persisted as `dsh.sidebar.view.v1`, default `chats`): the wide column renders a two-tab list above the New button and hands the active tab to the `sidebar.workspaces` owner, which renders the flat chat list or the Workspace tree. The wordmark and New button act on the active tab: Workspaces starts the runtime's page-local frontend Session Intent; Chats injects `startChat`, which reuses the current blank chat session or mints one with `agentPreset: 'chat'` and opens it. The rail has no tab control; its New icon follows the persisted tab.
...
`SidebarRootComponentProps` composes the layout owner share, the global `useSessions` and `useWorkspaces` hooks, the declared `sidebar.workspaces` and `sidebar.settings` child slots, the shell tab store, and injected `startSession`/`startChat` plus sidebar-toggle callbacks.
