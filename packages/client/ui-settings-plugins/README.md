...
The cards this package ships cover the shell executor (`bash`), the agent loop's tool-call parallelism (`agent-loop`), and the keyless websift search provider (`web-search-websift`).
...
A field's presence in the raw user layer — not its value — is what marks it overridden; a reset clears that field so it re-inherits the composition layer. A closed-choice field (the search card's backend) names a fixed set, and a draft outside that set blocks the save rather than writing a value the section cannot act on.
