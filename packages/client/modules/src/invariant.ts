/**
 * Package-owned invariant companion for `@open-harness/oh-client-modules`.
 * @module @open-harness/oh-client-modules/invariant
 */
...
const PACKAGE_NAME = '@open-harness/oh-client-modules'
  ...
  ctx.on('internal/plugin', () => {
    ...
    for (const row of host.graph().entries) {
      if (host.clientPath(row.id) === undefined) {
        fail(`web plugin graph row "${row.id}" advertises ${row.url} but resolves no client bundle path — the served __OH_BOOT__ would 404 on fetch`)
      }
    }
  }, { global: true })
}
