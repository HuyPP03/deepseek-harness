...
Candidate synthesis hides the host `/permission` row for chat sessions (summary `agentPreset: 'chat'`): the host refuses the switch for them, so the menu does not advertise it; a typed line still reaches the host and gets its error.
