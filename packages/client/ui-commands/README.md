# @open-harness/oh-client-ui-commands
...
`src/client/contract.ts` is the fixed business contract: `CommandUiContract.register(name, spec)`, `decorate(name, spec)`, and `menuRows(session, signal)` are everything a business package consumes; `CommandUiSpec{options, onSelect}` keeps popup data self-contained — the shell component belongs to this package and business packages never see it. `menuRows` reads the same merged view the slash menu shows (host catalog + available client contributions that pass `available`, in menu order, the chat-session `/permission` hide included) without position or query filtering, so a business popup can list what this session can run (the /help tool does). A contribution is a client-owned command (a host-name collision fails loud); a decoration adds a bare-invocation popup to an EXISTING host command. The host keeps its catalog row, argument claim (space / argued Enter), and lifecycle logging, and a decorated name with no host row in the session's directory never fires. Command kinds derive per dispatch, never per registration: a host descriptor with `input` is `leadingInput`, a registered `CommandUiSpec` is `popupSelect`, and everything else is `execute`.
...
Candidate synthesis hides host rows for the fixed surfaces: the `/permission` row for chat sessions (summary `agentPreset: 'chat'`) and provider chats (a session whose `agentPreset` the composed connectors' published preset set claims) — the host refuses the switch for both — and the `/mode` row for both, because neither surface's mode is switchable. `providerHidden` rows additionally stay hidden for provider chats. A typed line still reaches the host and gets its error.
