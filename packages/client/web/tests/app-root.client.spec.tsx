...
afterEach(cleanup)
import { AppRoot } from '@open-harness/oh-client-web/src/AppRoot.tsx'
import { createLoaderStatusStore, createSignal } from '@open-harness/oh-client-web/src/loader-status.ts'
...
describe('AppRoot', () => {
  ...
  it('lists failed entries and stays on the loading page', () => {
    ...
    act(() => {
      status.set('@open-harness/oh-client-ui-layout', 'failed')
      status.set('ok', 'active')
    })
    expect(getByText('Failed to load plugins')).toBeTruthy()
    expect(getByText('@open-harness/oh-client-ui-layout')).toBeTruthy()
    expect(queryByTestId('real-ui')).toBeNull()
  })
