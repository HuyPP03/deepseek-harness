...
import type { ClientContext, SessionFace } from '@open-harness/oh-client-runtime/client'
import { CHAT_PRESET_ID } from '@open-harness/oh-client-runtime/client'
import type { CommandUiContract, SelectOption } from '@open-harness/oh-client-ui-commands/client'
import type { ClientSessionContext } from '@open-harness/oh-client-ui-input-trigger/client'
import type { PermissionSelect } from '@open-harness/oh-permission-presets/client'
import { PermissionRow } from './PermissionRow.tsx'
import type { PermissionRowInjected } from './PermissionRow.tsx'
import {
  accessEn, accessVi, accessZh, en, vi, zh,
} from './locales.ts'
import {
...
export function apply(ctx: ClientContext): void {
  ...
  ctx.effect(() => {
    const disposers = [
      ...
      ctx.locale.register(ACCESS_NS, 'vi', {
        'confirm.title': accessVi['confirm.title'],
        'confirm.description': accessVi['confirm.description'],
        'confirm.acknowledge': accessVi['confirm.acknowledge'],
        'confirm.cancel': accessVi['confirm.cancel'],
        'confirm.enable': accessVi['confirm.enable'],
      }),
    ]
  ...
  ctx.effect(() => ctx.locale.register('settings.permission', { en, vi, zh }), 'ui-permission: settings row dictionaries')
