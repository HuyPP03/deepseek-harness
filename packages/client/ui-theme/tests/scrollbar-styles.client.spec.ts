 ...
 * --oh-alias-scrollbar-* token design-platform.css defines has a consumer,
...
const TOKEN_PREFIX = '--oh-alias-scrollbar-'
/** Prefix of the rebindable indirection scrollbar.css owns. */
const INDIRECTION_PREFIX = '--oh-scrollbar-'
...
const ELEVATED_REBIND = new Map([
  ['--oh-scrollbar-thumb', '--oh-alias-scrollbar-bg-l2'],
  ['--oh-scrollbar-thumb-hover', '--oh-alias-scrollbar-hover-l2'],
 ...
 * @returns every `--oh-*` token the sheet's rendering declarations depend on.
 */
function tokensRendered(rules: CssRule[]): Set<string> {
  ...
  while (pending.length > 0) {
    for (const name of varReferences(pending.pop()!)) {
      if (name.startsWith('--oh-')) reached.add(name)
 ...
 * so something a scrollbar can sit against. `--oh-alias-button-*`,
 * `--oh-alias-interactive-*`, and `--oh-alias-markdown-*` reach the same dark
 ...
 */
const SURFACE_TOKEN_PATTERN = /^--oh-(?:alias-bg-|specific-)/
...
function elevatedRungs(): Set<string> {
  ...
  const rungs = new Set([resolve('--oh-alias-bg-layer-2'), resolve('--oh-alias-bg-layer-3')])
    ...
    for (const rule of platformRules) {
      for (const [property, value] of rule.declarations) {
        ...
        for (const reference of varReferences(value)) {
          expect(reference, `${property}: ${value}`).toMatch(/^--oh-static-/)
        }
      }
    }
  })
...
describe('scrollbar.css width variable', () => {
  ...
  it('every reader of the width variable outside ui-theme references a defined variable', () => {
    ...
    // (`right: var(--oh-scrollbar-width)`); a rename in scrollbar.css without
...
describe('elevated surface rebinds', () => {
  ...
  it('rebinds the pair to one target: the l2 elevation pair, or transparent', () => {
    ...
    // `thumb-hover: var(--oh-alias-scrollbar-hover-l2)` — which repaints the
    ...
    // A shape check admits `color-mix(…, var(--oh-alias-scrollbar-bg-l2) 85%,
  ...
  it('resolves the elevated surface set from the palette ladder', () => {
    ...
    // defined. `--oh-specific-tip` is the regression that proved the point: it
    ...
    expect(elevatedSurfaces).toContain('--oh-alias-bg-layer-2')
    expect(elevatedSurfaces).toContain('--oh-alias-bg-layer-3')
    expect(elevatedSurfaces).toContain('--oh-specific-menu')
    expect(elevatedSurfaces).toContain('--oh-specific-input-major')
    expect(elevatedSurfaces).toContain('--oh-specific-tip')
    // Base surfaces stay out, or every scroll container would be in scope and
    // the check would say nothing.
    expect(elevatedSurfaces).not.toContain('--oh-alias-bg-base')
    expect(elevatedSurfaces).not.toContain('--oh-alias-bg-layer-1')
  })
  ...
  it('every sheet that scrolls on an elevated surface rebinds', () => {
    ...
    // `--oh-alias-bg-*` and `--oh-specific-*` name a surface, so a floating
