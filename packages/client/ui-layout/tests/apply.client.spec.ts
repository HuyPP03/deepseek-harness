...
import { apply as themeApply, inject as themeInject, ThemeRuntime } from '@open-harness/oh-client-ui-theme/client'
import { apply, inject, LayoutController } from '@open-harness/oh-client-ui-layout/client'
import { apply as nodeApply } from '@open-harness/oh-client-ui-layout'
import * as invariant from '@open-harness/oh-client-ui-layout/invariant'
...
describe('ui-layout client apply', () => {
  ...
  it('provides ctx.layout and registers AppFrame into root with the three child declarations', async () => {
    ...
    // …and declared the frame children in the ledger.
    expect(slots.spec('sidebar')).toEqual({ kind: 'single', scope: 'root' })
    expect(slots.spec('conversation')).toEqual({ kind: 'single', scope: 'session-maybe' })
    expect(slots.spec('main.connectors')).toEqual({ kind: 'single', scope: 'root' })
  ...
  it('theme presenter applies the initial snapshot, follows theme/change, and unwinds on dispose', async () => {
    ...
    expect(document.body.hasAttribute('data-oh-dark-theme')).toBe(true)
...
describe('node half + invariant companion', () => {
  ...
  it('invariant companion registers under the package name', async () => {
    ...
    expect(register).toHaveBeenCalledWith('@open-harness/oh-client-ui-layout', expect.any(Function))
