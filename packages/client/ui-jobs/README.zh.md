...
Web 后台任务特性的归属方：向 `conversation.session.header.actions` 贡献一个条目，列出当前会话可见的 `ctx.jobs` 记录，并占据详情面板为被点击行渲染的 `conversation.details.job` 座位。列表数据完全来自 [`dsh-client-runtime`](../runtime/README.md) 从 `session/jobs` 帧折叠出的 `jobsBySession` 列表镜像；日志字节则来自连接的 `jobs.log` 单射。
