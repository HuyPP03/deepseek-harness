...
Web background-job feature owner: contributes one entry to `conversation.session.header.actions` listing the `ctx.jobs` records this session can see, and occupies the `conversation.details.job` seat the details panel renders for a clicked row. The list data arrives entirely through the `jobsBySession` list mirror that [`oh-client-runtime`](../runtime/README.md) folds from `session/jobs` frames; the log bytes arrive through the connection's `jobs.log` unary.
...
None, as this package renders host-computed registry state for a human and touches no prompt, message, schema, stream, or tool result. The model's own view of the same jobs stays with [`oh-tool-jobs`](../../jobs/tool-jobs/README.md).
