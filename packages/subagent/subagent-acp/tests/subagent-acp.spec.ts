...
import type { SubprocessOutcome } from '@open-harness/oh-subprocess'
...
describe('child env layering (through the subprocess seam)', () => {
  ...
  it('forwards explicit OH_* config entries to the child', async () => {
    // A deployment sets child-harness facts like OH_PERMISSION_MODE in
    ...
    const ctx = await setup({ MOCK_ECHO_ENV: 'OH_ACP_TEST_FACT', OH_ACP_TEST_FACT: 'managed' })
...
describe('cwd resolution', () => {
  ...
  it('resolves a relative config cwd against the launch directory at load', async () => {
    ...
    // ABSOLUTE form — Open Harness's own ACP server rejects a relative session cwd, and
