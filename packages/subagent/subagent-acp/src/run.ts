 ...
 * @module @open-harness/oh-subagent-acp/run
 */
...
import { AssistantOutputFold } from '@open-harness/oh-subagent'
import type { SubagentResult, SubagentRun, SubagentStartRequest, SubagentStopReason } from '@open-harness/oh-subagent'
import type { SubprocessHandle, SubprocessSpawnSpec } from '@open-harness/oh-subprocess'
...
export interface AcpRunSpec {
   ...
   * Explicit `OH_*` entries are deployment-owned facts for the child harness
   * (e.g. `OH_PERMISSION_MODE`); they simply merge after the scrub that
...
export async function startAcpRun(request: SubagentStartRequest, spec: AcpRunSpec): Promise<SubagentRun> {
  ...
  // to the result. The seam's scrub drops ambient credentials and OH_* names
