...
import ToolRuntime, { TOOL_ABORTED_BEFORE_DISPATCH } from '@open-harness/oh-tools'
import { assembleContextFor, type Agent } from '@open-harness/oh-agent'
...
import type { SubagentStartRequest } from '@open-harness/oh-subagent'
import LocalJobRegistry from '@open-harness/oh-jobs-local'
import * as SubagentSpawn from '@open-harness/oh-subagent-spawn-in-process'
...
/**
 * Drives the REAL plugin body: mounts `oh-tool-subagent` on a real
...
describe('oh-tool-subagent background mode', () => {
  ...
  it('fails loud when the tasks runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @open-harness/oh-jobs')
  })
...
describe('oh-tool-subagent continuable background mode', () => {
  const roots: string[] = []
  afterEach(() => {
  ...
  async function continuableSetup() {
    ...
    const root = mkdtempSync(path.join(tmpdir(), 'oh-tool-subagent-continuable-'))
    roots.push(root)
