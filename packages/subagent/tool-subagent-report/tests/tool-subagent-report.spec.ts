...
import * as control from '@open-harness/oh-tool-subagent-control'
...
async function setup(options: { load?: boolean; config?: tool.Config } = {}) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'oh-tool-subagent-report-'))
...
describe('oh-tool-subagent-report', () => {
...
describe('oh-tool-subagent-report result independence', () => {
