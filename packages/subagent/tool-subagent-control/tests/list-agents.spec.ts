...
import type { SubagentListEntry } from '@open-harness/oh-subagent'
...
async function setupWith(adapter: MockAdapter | GatedAdapter) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'oh-tool-list-agents-'))
  roots.push(root)
...
describe('oh-tool-subagent-control/list-agents', () => {
