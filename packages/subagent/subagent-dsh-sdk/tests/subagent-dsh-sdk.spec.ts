...
describe('oh-subagent-dsh-sdk provider', () => {
  ...
  it('scrubs ambient credentials but forwards explicit config env', async () => {
    process.env.OH_TEST_AMBIENT_SECRET_KEY = 'leak-me-not'
    try {
      const ctx = await setup({
        FAKE_ECHO_ENV: 'OH_TEST_AMBIENT_SECRET_KEY,DEEPSEEK_API_KEY',
      ...
      expect(answer).toContain('OH_TEST_AMBIENT_SECRET_KEY=\n')
      expect(answer).toContain('DEEPSEEK_API_KEY=explicit-child-key')
      await run.dispose()
      await ctx.fiber.dispose()
    } finally {
      delete process.env.OH_TEST_AMBIENT_SECRET_KEY
    }
  })
