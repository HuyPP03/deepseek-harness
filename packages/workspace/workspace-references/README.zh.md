...
通过 `ctx.workspaceReferences`（[`WorkspaceReferenceService`](src/index.ts)）提供会话引用项目：会话除自身工作区（其 `header.cwd`，即主项目）之外，可附加用于对照的项目目录。
...
引用目录在任何沙箱模式下都可读：所有受限后端（bubblewrap、Landlock、Seatbelt、Windows ACL 受限令牌）都允许读取会话工作区之外的文件系统，因此被接纳的引用目录可被 fs 工具与沙箱执行直接读取。可写性由现行沙箱模式决定：`read-only` 与 `workspace-write` 仍只允许写入会话工作区（加平台临时目录），而 `workspace-refs-write` 把引用目录（策略上的 `referenceRoots`）并入可写集合；本包不附带任何沙箱或文件系统 provider 变更。
...
会话存在引用时，每次组装含一个 `workspace:references` system prompt 段（order 115，在 `sandbox:policy` 之后）；集合为空时没有该段。不新增工具：引用通过会话已有的文件工具访问。
