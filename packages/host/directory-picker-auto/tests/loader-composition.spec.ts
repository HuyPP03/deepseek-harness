...
import HttpServer from '@open-harness/oh-host-webserver'
import type { DirectoryPicker } from '@open-harness/oh-host-directory-picker'
import BrowseDirectoryPicker from '@open-harness/oh-host-directory-picker-browse'
import NativeDirectoryPicker from '@open-harness/oh-host-directory-picker-native'
...
const AUTO = '@open-harness/oh-host-directory-picker-auto'
const NATIVE = '@open-harness/oh-host-directory-picker-native'
const BROWSE = '@open-harness/oh-host-directory-picker-browse'
const NATIVE_SURFACE = '@open-harness/oh-client-ui-directory-picker-native'
const BROWSE_SURFACE = '@open-harness/oh-client-ui-directory-picker-browse'
...
async function loadComposition(
  ...
  root = await mkdtemp(join(tmpdir(), 'oh-directory-picker-auto-'))
  const configPath = join(root, 'cordis.yml')
  await writeFile(configPath, [
    "- name: '@open-harness/oh-host-webserver'",
    '  config:',
    `    host: '${bindHost}'`,
    '    port: 0',
    `- name: '${AUTO}'`,
  ...
  const modules = new Map<string, unknown>([
    ['@open-harness/oh-host-webserver', HttpServer],
...
function stubAttendedHost(): void {
  fakeBin = mkdtempSync(join(tmpdir(), 'oh-picker-bin-'))
