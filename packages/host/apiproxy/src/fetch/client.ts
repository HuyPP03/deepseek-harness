...
import {
  ...
  sessionSetReferencesValueSchema,
...
import { fileListValueSchema, fileReadValueSchema } from '../api/files.schema.ts'
import {
...
import {
  ...
  connectorDisconnectValueSchema,
  connectorListViewSchema,
  connectorRemoveValueSchema,
} from '../api/connector.schema.ts'
import { llmDiscoverModelsValueSchema, llmModelsValueSchema, llmProvidersValueSchema } from '../api/llm.schema.ts'
import {
  mcpAddValueSchema, mcpListValueSchema, mcpReconnectValueSchema, mcpRemoveValueSchema,
...
export interface IApiClient {
  sessions: {
    ...
    setReferences(payload: RequestPayload<'session.setReferences'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'session.setReferences'>>>
  ...
  connectors: {
    list(payload: RequestPayload<'connector.list'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'connector.list'>>>
  ...
  mcp: {
    ...
    add(payload: RequestPayload<'mcp.add'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'mcp.add'>>>
    remove(payload: RequestPayload<'mcp.remove'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'mcp.remove'>>>
    reconnect(payload: RequestPayload<'mcp.reconnect'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'mcp.reconnect'>>>
  }
...
const UNARY_VALUE_SCHEMAS: { [K in keyof RpcMethodMap]: z.ZodType<Wire<ResponseValue<K>>> } = {
  ...
  'session.setReferences': sessionSetReferencesValueSchema,
  ...
  'files.list': fileListValueSchema,
  'files.read': fileReadValueSchema,
  ...
  'connector.list': connectorListViewSchema,
  ...
  'mcp.add': mcpAddValueSchema,
  'mcp.remove': mcpRemoveValueSchema,
  'mcp.reconnect': mcpReconnectValueSchema,
}
...
const INTERNAL_BASE = 'http://oh.internal'
...
export abstract class AbstractApiClient implements IApiClient {
  ...
  readonly sessions: IApiClient['sessions'] = {
    ...
    setReferences: (payload, signal) => this.callUnary('session.setReferences', payload, signal),
  ...
  readonly connectors: IApiClient['connectors'] = {
    list: (payload, signal) => this.callUnary('connector.list', payload, signal),
  ...
  readonly mcp: IApiClient['mcp'] = {
    ...
    add: (payload, signal) => this.callUnary('mcp.add', payload, signal),
    remove: (payload, signal) => this.callUnary('mcp.remove', payload, signal),
    reconnect: (payload, signal) => this.callUnary('mcp.reconnect', payload, signal),
  }
