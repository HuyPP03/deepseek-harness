...
`files.list` 服务于 composer 的 `@` 引用（mention）source：宿主从会话头与日志解析会话的项目 cwd 及其已附着的 reference 项目，在协议的固定边界内遍历（深度 8、至多 20000 个已扫描条目、至多 100 行结果）——同时列出文件**与目录**，应用噪声规则（所有点前缀条目，加上 `src/files-skip.json` 中的目录 basename 清单：依赖缓存、虚拟环境、构建产物），绝不跟随符号链接。可选的 `query` 是遍历自身枚举的过滤串，绝不是宿主会解析的路径：它大小写不敏感地过滤并排序（basename 前缀 优于 路径前缀 优于 子串），以结尾斜杠结尾时把遍历剪枝到经过验证的相对前缀，并先列出前缀目录本身。客户端从不提交路径，列表不创建或恢复任何 Agent；`truncated` 报告扫描边界。
...
间接影响，经由模型选择：`session.selectModel` 与网关的 `/effort` 命令持久化完整的 `ModelSelection`（包括由适配器持有的推理强度），Host 会在下一次提示词组装边界对其进行快照，因此后续请求采用所选提供方、模型与推理强度；该包本身既不组装也不发送提供方请求。

#### KV Cache 影响

切换路由或推理强度可能减少或失效提供方侧后续请求的缓存复用；提示词前缀本身不受影响。
