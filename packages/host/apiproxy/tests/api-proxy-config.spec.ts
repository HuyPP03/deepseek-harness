...
import LlmRuntime, { LlmAdapter } from '@open-harness/oh-llm'
import type { GenerateOptions, LlmModelInfo, LlmProviderInfo, StreamChunk } from '@open-harness/oh-llm'
import { SettingsProvider, settingsNamespace } from '@open-harness/oh-settings'
import type { SettingsNamespace } from '@open-harness/oh-settings'
import { CredentialProvider } from '@open-harness/oh-credentials'
import type { CredentialInfo, CredentialRef, ResolvedCredential } from '@open-harness/oh-credentials'
import type { HostFrame } from '../src/api/index.ts'
import type { RpcRequest, RpcResponse } from '../src/api/rpc.ts'
import { RpcId } from '../src/api/rpc.ts'
import { AGENT_DEFAULT_MODEL_SETTINGS_NAMESPACE } from '@open-harness/oh-agent-default-model'
...
describe('settings domain', () => {
  it('reports an actionable error when no settings provider is mounted', async () => {
    ...
    expect(error.message).toContain('oh-settings-file')
  })
...
describe('credentials domain', () => {
  it('reports an actionable error when no credential provider is mounted', async () => {
    ...
    expect(error.code).toBe('internal')
    expect(error.message).toContain('oh-credentials-local')
  })
