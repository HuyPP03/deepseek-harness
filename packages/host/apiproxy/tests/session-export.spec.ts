...
import type { SessionLineageNode } from '@open-harness/oh-session-query'
import type { SessionRawArtifact } from '@open-harness/oh-session-persistence'
import ApiProxyService, { createApiProxy, toFetchHandler } from '@open-harness/oh-host-apiproxy'
...
describe('session.export download endpoint', () => {
  it('streams a ZIP with the root artifact verbatim under its original filename', async () => {
    ...
    expect(response.headers.get('content-disposition')).toContain('oh-session-session-root.zip')
