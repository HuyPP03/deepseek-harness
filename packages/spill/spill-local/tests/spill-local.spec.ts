...
import type { SaveTextSpill } from '@open-harness/oh-spill'
import LocalSpillStore, { encodeSegment, privateRoot, saveTextFile, sessionDir } from '@open-harness/oh-spill-local'

let root: string

beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), 'oh-spill-test-'))
})
afterEach(() => {
