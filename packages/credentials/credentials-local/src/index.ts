/**
 * File-backed credentials provider over `$OH_HOME/.credentials.yaml`, layered
 * against the environment by how much each layer is trusted:
 *
 * ```text
 * inherited process environment      (read-only, wins)
 * > $OH_HOME/.credentials.yaml      (provider-managed, writable)
...
/** File-backed credentials provider (`$OH_HOME/.credentials.yaml`). */
