...
npm scope 为 `@open-harness/oh-*`；Cordis `Service` 子类和函数插件通过 `ctx.effect()`、`ctx.on()` 或 `ctx.waterfall()` 注册。规则见[包](AGENTS.md)与[根规则](../AGENTS.md#conventions)。

## 层级结构

包按组置于 `packages/<group>/<pkg>/`；包名仍为 `@open-harness/oh-<pkg>`。**组 README 负责包／ctx 键映射。**
...
| [`mcp/`](mcp/README.md) | MCP 桥接：注册工具的客户端、存活服务器注册表和 `/mcp` 状态命令 | 产品：稳定 API |
...
| [`bundle/`](bundle/README.md) | 可安装的 `oh --profile` 补丁层 | 产品：稳定 API |
...
| [`connectors/`](connectors/README.md) | 预定义外部连接：基于 MCP 服务器的连接器目录 + 状态机 | 产品：稳定 API |
...
| [`util/`](util/README.md) | 组间共享的零依赖工具（`Branded<B>`、Harness home／路径辅助函数、超时、留存） | 支持：小型、稳定、无 harness 依赖 |
...
**扩展插件依赖 Service Definition，绝不依赖具体提供方。** `oh-agent-loop` 可替换；UI、钩子和工具插件使用 `oh-agent`。包括 `oh-agent-spine-demo` 在内的组合包可以依赖主干插件。能力会将需要独立演进的 Service Definition／Service Provider／Consumer 角色分离；详见[能力 seam](../.agents/notes/implemented/architecture/2026-06-13-capability-seams.md)。
