...
Fixed surfaces: `Config.chatPresetIds` lists the agent-preset ids whose sessions are chat sessions (the web surface ships `chat`); empty (the default) disables that half. A session created under a listed preset — or under a preset the composed connectors service claims (a provider session; the claim is read through `ctx.get('connectors').presetIds()`, absent when no connectors service is composed) — pins `Config.chatPreset` (default `read-only`, validated against the table) instead of the default preset, and the `/permissionPresets` switch refuses any session still running a listed or claimed preset — its mode is fixed by composition, and client surfaces hide the switch for it rather than surfacing a refused action.
