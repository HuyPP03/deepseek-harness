/**
 * The bundle's substance is its patch file: the `oh.bundle.patch` manifest
...
describe('dsh-base bundle', () => {
  it('declares a parseable patch list through the oh.bundle.patch manifest field', () => {
    ...
    expect(rows.find(row => row.id === 'session-telemetry-otel')?.config?.['mode']).toEqual({
      __jsExpr: "process.env.OH_TELEMETRY_MODE || 'DISABLED'",
    })
    expect(rows.filter(row => row.id === 'subagent-codex')).toHaveLength(0)
    expect(rows.filter(row => row.id === 'subagent-claude-code')).toHaveLength(0)
    expect(manifest.dependencies).not.toHaveProperty('@open-harness/oh-subagent-codex')
    expect(manifest.dependencies).not.toHaveProperty('@open-harness/oh-subagent-claude-code')
  })
