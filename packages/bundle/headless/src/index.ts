 ...
 *
 * @module @open-harness/oh-headless
 */
...
function fail(io: HeadlessIo, error: unknown): void {
  io.stderr.write(`oh: ${error instanceof Error ? error.message : String(error)}\n`)
  io.exit(1)
}
...
async function run(ctx: Context, task: string, io: HeadlessIo): Promise<void> {
  ...
  if (outcome.reason?.kind === 'error') {
    io.stderr.write(`oh: ${outcome.reason.error.code}: ${outcome.reason.error.message}\n`)
  }
