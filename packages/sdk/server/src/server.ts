...
import { carrierKeyOf, type Scoped } from '@open-harness/oh-scope'
import { SessionId } from '@open-harness/oh-session'
import type SubagentRuntime from '@open-harness/oh-subagent'
import type { SubagentRunEndInfo } from '@open-harness/oh-subagent'
...
export class HarnessSdkJsonRpcServer {
  ...
  async initialize(params: InitializeParams): Promise<InitializeResult> {
    ...
    return { serverInfo: { name: 'open-harness-sdk-runtime', version: '0.0.1' } }
  }
  ...
  async handleRequest(method: string, params: Record<string, unknown> | undefined): Promise<unknown> {
    switch (method) {
      ...
      default:
        throw new Error(`unknown Open Harness SDK runtime method: ${method}`)
    }
  }
