...
import SubagentRuntime, { type SubagentResult, type SubagentRunEndInfo } from '@open-harness/oh-subagent'
import type { JsonRpcTransportPeer } from '@open-harness/oh-sdk-protocol'
...
describe('HarnessSdkJsonRpcServer', () => {
  it('creates a harness agent and calls the configured OpenAI-compatible endpoint', { timeout: 15_000 }, async () => {
    ...
    try {
      ...
      expect(init.serverInfo.name).toBe('open-harness-sdk-runtime')
  ...
  it('rejects unknown JSON-RPC runtime methods', async () => {
    ...
    try {
      ...
      await expect(server.handleRequest('does/not/exist', {}))
        .rejects
        .toThrow('unknown Open Harness SDK runtime method: does/not/exist')
