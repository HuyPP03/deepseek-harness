/**
 * Named wire types for the Open Harness SDK runtime protocol: the three
 * request/result pairs and the four server-to-client notification payloads
 * exchanged over the newline-delimited JSON-RPC stdio transport. The server
 * plugin (`@open-harness/oh-sdk-jsonrpc-server`) and SDK clients share these shapes;
 * `serverInfo.name` stays the wire-stable `open-harness-sdk-runtime`.
 *
 * @module @open-harness/oh-sdk-protocol/types
 */

import type { ContentBlock } from '@open-harness/oh-llm'
import type { SessionEvent } from '@open-harness/oh-session'
import type { SubagentStopReason } from '@open-harness/oh-subagent'
...
export interface InitializeResult {
  /** Wire-stable server identity (`open-harness-sdk-runtime`) and version. */
