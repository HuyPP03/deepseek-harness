...
import type { ShellExecRequest, ShellExecSpec, ShellExecutor, ShellRunResult } from '@open-harness/oh-shell'
import { DEFAULT_HOOK_TIMEOUT_MS, runHook } from '@open-harness/oh-hook-protocol'
import type { RunHookOptions } from '@open-harness/oh-hook-protocol'
 ...
 * executor (oh-bash-local) is exercised end-to-end by the hook-bridge plugins
...
function recordingBash(run: (spec: ShellExecSpec) => Promise<ShellRunResult>): {
  ...
  const bash = {
    resolve(request: ShellExecRequest): ShellExecSpec {
      ...
      // exactly what oh-bash-local's resolve does for the fields runHook sets.
      return {
