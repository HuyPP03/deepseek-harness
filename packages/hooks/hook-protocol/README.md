# @open-harness/oh-hook-protocol

English | [中文](README.zh.md)

The **shared core** of the Claude Code / Codex hook wire protocol. NOT a cordis plugin — it registers nothing and injects nothing. It is a **library** of dialect-neutral primitives the two bridge plugins (`@open-harness/oh-hooks-claude-code`, `@open-harness/oh-hooks-codex`) import so neither re-implements the identical halves of the protocol.
...
| Concern | Here (`oh-hook-protocol`) | The bridge (`oh-hooks-claude-code` / `-codex`) |
|---|---|---|
...
Indirectly, through `oh-hooks-claude-code` and `oh-hooks-codex`, which can turn parsed hook output into prompt context, blocked outcomes, or continuation feedback.
