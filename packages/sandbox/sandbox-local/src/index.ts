 ...
 * @module @open-harness/oh-sandbox-local
 */
...
import { AclWriteGrant, assertTempRootOutsideWorkspace, refWriteSid, tempWriteSid, workspaceWriteSid } from '@open-harness/oh-sandbox-windows-acl'
...
const PLATFORM_CHAINS: Record<string, readonly SelectedRunner['runner'][]> = {
  ...
  darwin: ['seatbelt'],
  // The Windows restricted-token runner (@open-harness/oh-sandbox-windows-acl):
...
export class LocalSandboxProvider extends SandboxProvider {
  ...
  private windowsAclRunnerInvocation(): string[] {
    ...
    const builtEntry = this.internals.windowsAclRunnerEntry ?? fileURLToPath(import.meta.resolve('@open-harness/oh-sandbox-windows-acl/runner'))
    if (existsSync(builtEntry)) return [process.execPath, builtEntry]
    const sourceEntry = fileURLToPath(import.meta.resolve('@open-harness/oh-sandbox-windows-acl/src/runner.ts'))
