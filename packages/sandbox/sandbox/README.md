...
Implementations: [`@open-harness/oh-sandbox-local`](../sandbox-local/) (Linux: `bwrap`, else the per-platform Landlock launcher; macOS: `sandbox-exec`/Seatbelt). Consumers: [`@open-harness/oh-bash-sandbox`](../../shell/bash-sandbox/) (wraps `['bash', '-c', command]`).
