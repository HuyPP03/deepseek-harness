...
const CHOOSER_PACKAGE = '@open-harness/oh-host-directory-picker-auto'
...
const CHOOSER_BACKEND_PACKAGES = [
  '@open-harness/oh-host-directory-picker-native',
  '@open-harness/oh-host-directory-picker-browse',
  '@open-harness/oh-client-ui-directory-picker-browse',
  '@open-harness/oh-client-ui-directory-picker-native',
]
 ...
 * `oh.client` block, and the node half of a surface plugin is an empty
 ...
 * `oh.client` declaration disagree.
 */
function validateClientHalvesDeclared(): string[] {
  return globSync('packages/client/*/package.json', { cwd: root }).flatMap((manifestPath) => {
    ...
    const declaresClient = manifest.oh?.client !== undefined
    if (shipsClient === declaresClient) return []
    return [shipsClient
      ? `${manifestPath}: exports "./client" but declares no oh.client, so its browser half is never served`
      : `${manifestPath}: declares oh.client but exports no "./client" entry to serve`]
  })
}
 ...
 *
 * Both have happened. `shell-env` in a preset realm left `OH_WEB_URL` reaching
