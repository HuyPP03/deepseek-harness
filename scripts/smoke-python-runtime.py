...
if TYPE_CHECKING:
    from open_harness import RunResult
...
- id: sessions
  ...
  config:
    root: !!js process.env.OH_SESSION_ROOT
    compression: 'none'
- id: code-runtime
  name: '@open-harness/oh-code-runtime-worker-thread'
- id: subagents
  name: '@open-harness/oh-subagent'
- id: subagent-spawn-in-process
  name: '@open-harness/oh-subagent-spawn-in-process'
  config:
    providerName: spawn
- id: subagent-tool
...
- id: cordis-host-runner
  name: '@open-harness/oh-cordis-host-runner'
- id: cordis-tool
  name: '@open-harness/oh-tool-cordis'
"""
...
def smoke_direct(base_url: str, executable: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="oh-direct-") as temporary:
        ...
        environment = {
            **os.environ,
            "OH_CORDIS_CONFIG": str(cordis),
            "OH_SESSION_ROOT": str(sessions),
            "OH_CWD": str(root),
