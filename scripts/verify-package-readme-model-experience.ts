...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/shell/shell': { kind: 'indirect', reason: 'The service interface delegates all model rendering to oh-tool-bash.' },
  'packages/shell/shell-env': { kind: 'indirect', reason: 'The env service exposes managed OH_* facts through the shell tools (oh-tool-bash/oh-tool-pwsh); it registers no prompt or schema of its own.' },
  'packages/shell/bash-local': { kind: 'indirect', reason: 'The executor backend delegates model rendering to oh-tool-bash.' },
  'packages/shell/pwsh-local': { kind: 'indirect', reason: 'The executor backend delegates model rendering to oh-tool-pwsh.' },
  'packages/code-runtime/code-runtime': { kind: 'indirect', reason: 'The service interface delegates model rendering to Code Mode in oh-tools.' },
  'packages/core/agent-tool-presentation': { kind: 'indirect', reason: 'The row only selects between the two projections oh-tools owns; it registers no prompt, schema, or result of its own.' },
  'packages/code-runtime/code-runtime-worker-thread': { kind: 'indirect', reason: 'The worker backend delegates model rendering to Code Mode in oh-tools.' },
  ...
  'packages/connectors/connectors': { kind: 'none', reason: 'The catalog and state machine register no model context; a connected connector\'s servers are ordinary MCP tools owned by mcp-client, and the preset owns any model-facing wiring.' },
  'packages/connectors/oauth-flow': { kind: 'none', reason: 'The engine is a host-side auth driver; it registers no prompt, schema, or result of its own, and the connected servers it enables are ordinary MCP tools owned by mcp-client.' },
  'packages/connectors/device-flow': { kind: 'none', reason: 'The engine is a host-side auth driver; it registers no prompt, schema, or result of its own, and the connected servers it enables are ordinary MCP tools owned by mcp-client.' },
  'packages/credentials/oauth-tokens': { kind: 'none', reason: 'A passive file store: bundles never enter a prompt, schema, or result, and the connector auth flow that stores them owns any model-visible use.' },
  ...
  'packages/client/ui-connectors': { kind: 'none', reason: 'Browser-side connector roster; the host state machine and its mounted MCP servers own model-facing effects.' },
  'packages/client/ui-file-inspector': { kind: 'none', reason: 'Browser-side file inspector; renders logged diff material and files served by the raw channel without changing model context.' },
  ...
  'packages/client/ui-jobs': { kind: 'none', reason: 'Browser-side read-only projection of ctx.jobs records; oh-tool-jobs owns the model-facing behavior.' },
  ...
  'packages/client/ui-mcp': { kind: 'indirect', reason: 'Browser-side MCP surface; the servers a user adds own their model-visible tools through the mcp-client instances the host mounts.' },
  ...
  'packages/client/ui-plan': { kind: 'indirect', reason: 'The chip dispatches /plan off; oh-plan-mode owns the model-visible policy, exit tool, and logged state.' },
  'packages/client/ui-user-questions': { kind: 'indirect', reason: 'The package mounts oh-tool-ask-user; that tool owns the model-visible schema and answer rendering.' },
  ...
  'packages/fs/fs': { kind: 'indirect', reason: 'The service interface delegates model rendering to oh-tool-fs.' },
  'packages/e2b/fs-e2b': { kind: 'indirect', reason: 'The provider backend delegates model rendering to oh-tool-fs.' },
  'packages/fs/fs-local': { kind: 'indirect', reason: 'The provider backend delegates model rendering to oh-tool-fs.' },
  'packages/hooks/hook-protocol': { kind: 'indirect', reason: 'Only the hook bridge plugins render decoded hook output to a model.' },
  'packages/host/apiproxy': { kind: 'indirect', reason: 'The selectModel and /effort wire verbs persist the ModelSelection the Host snapshots at the next prompt-assembly boundary; the selected provider and effort own the model-visible effect.' },
  'packages/mcp/mcp-manager': { kind: 'indirect', reason: 'The manager mounts and unmounts mcp-client instances; the tools they discover own their model-visible effect.' },
  ...
  'packages/lsp/lsp': { kind: 'indirect', reason: 'The provider registry delegates model rendering to oh-tool-lsp.' },
  'packages/lsp/lsp-stdio': { kind: 'indirect', reason: 'The provider backend delegates model rendering to oh-tool-lsp.' },
  ...
  'packages/sandbox/sandbox-local': { kind: 'indirect', reason: 'The provider backend delegates model rendering to oh-bash-sandbox and oh-tool-bash.' },
  ...
  'packages/skill/skill': { kind: 'indirect', reason: 'The provider registry delegates model rendering to oh-tool-skill.' },
  'packages/skill/skill-badge': { kind: 'indirect', reason: 'The bundled provider delegates model rendering to oh-tool-skill.' },
  'packages/skill/skill-filesystem': { kind: 'indirect', reason: 'The provider backend delegates model rendering to oh-tool-skill.' },
  ...
  'packages/jobs/jobs-local': { kind: 'indirect', reason: 'The registry backend delegates model rendering to producer plugins and oh-tool-jobs.' },
  'packages/examples/acp-demo': { kind: 'indirect', reason: 'The app bundle delegates request composition to oh-agent-spine-demo and oh-acp.' },
  ...
  'packages/interaction/permission-presets': { kind: 'indirect', reason: 'The service writes mechanism events rendered by oh-user-approval and oh-tool-bash.' },
  ...
  'packages/web/web': { kind: 'indirect', reason: 'The provider registry delegates model rendering to oh-tool-web.' },
  'packages/web/web-fetch-http': { kind: 'indirect', reason: 'The provider backend delegates model rendering to oh-tool-web.' },
  'packages/web/web-fetch-websift': { kind: 'indirect', reason: 'The provider backend delegates model rendering to oh-tool-web.' },
  'packages/web/web-search-exa': { kind: 'indirect', reason: 'The provider backend delegates model rendering to oh-tool-web.' },
  'packages/web/web-search-websift': { kind: 'indirect', reason: 'The provider backend delegates model rendering to oh-tool-web.' },
