...
const vendoredPackages = new Set([
  '@open-harness/cordis',
  '@open-harness/cosmokit',
  '@open-harness/schemastery',
  '@open-harness/cordis-plugin-loader',
  '@open-harness/cordis-plugin-include',
  '@open-harness/cordis-plugin-group',
  '@open-harness/cordis-plugin-timer',
  '@open-harness/cordis-plugin-hmr',
  '@open-harness/cordis-plugin-logger-console',
])
const publicLandlockPackages = new Set([
  '@open-harness/node-addon-landlock-run',
  '@open-harness/node-addon-landlock-run-linux-arm64',
  '@open-harness/node-addon-landlock-run-linux-x64',
])
...
const publicationSourceAllowlist: Readonly<Record<string, readonly string[]>> = {
  '@open-harness/node-addon-landlock-run': ['src/main.c'],
}
...
const appPackageFiles: Readonly<Record<string, readonly string[]>> = {
  ...
  '@open-harness/oh-web-frontend': ['dist', '!dist/**/*.map'],
}
...
const packageFileExtras: Readonly<Record<string, readonly string[]>> = {
  // Profile bundles publish their oh.bundle.patch layer beside the lib.
  '@open-harness/oh-base': ['cordis.patch.yml'],
  '@open-harness/oh-web-app': ['cordis.patch.yml'],
  '@open-harness/oh-headless': ['cordis.patch.yml'],
  '@open-harness/oh-client-ui-theme': ['lib/styles'],
  // The Python runtime uses a distinct closed-resolution bin; the public CLI
  // keeps config-owned bare-package resolution through lib/bin.js.
  '@open-harness/oh-sdk-jsonrpc-demo': ['lib/packaged-bin.js'],
  // The argv-prefix runner entry ships beside the lib as its own bundle;
  // sandbox-local resolves it through the package's ./runner export. tsdown
  // also shares its generated FFI code through a hashed runtime chunk.
  '@open-harness/oh-sandbox-windows-acl': ['lib/runner.js', 'lib/types-*.js'],
  '@open-harness/oh-skill-badge': ['assets'],
  '@open-harness/oh-subprocess-local': ['scripts/ensure-spawn-helper.mjs'],
}
...
function checkWorkspace({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  if (isPublicLandlockPackage) {
    ...
    // them, while the oh family stays restricted until its own sequence goes
  ...
  if (manifest.name === 'oh' || manifest.name?.startsWith('@open-harness/') || manifest.name?.startsWith('@open-harness/')) {
  ...
  if (dir.startsWith('apps/') && (manifest.name === 'oh' || manifest.name?.startsWith('@open-harness/'))) {
  ...
  if (dir.startsWith('packages/') && manifest.name?.startsWith('@open-harness/oh-')) {
    const peer = manifest.peerDependencies?.['@open-harness/cordis']
...
function checkRepositoryVersion(): string[] {
  // The root carries the oh release family's version, so a prerelease such as
  // 0.0.1-rc.1 is a valid state between `release:oh` and its publication.
