...
function workspaceFixture(options: {
  ...
  const root = mkdtempSync(join(tmpdir(), 'oh-project-reference-faces-'))
  roots.push(root)
  ...
  writeJson(join(shared, 'package.json'), { name: '@open-harness/oh-shared' })
  writeJson(join(shared, 'tsconfig.json'), {
    extends: '../../../tsconfig.base.json',
    references: [],
  })
  writeJson(join(split, 'package.json'), { name: '@open-harness/oh-split' })
...
describe('Project Reference compiler faces', () => {
  ...
  it('uses the referencing project face throughout the reachable graph', () => {
    ...
    writeJson(join(hostConsumer, 'package.json'), { name: '@open-harness/oh-host-consumer' })
    ...
    writeJson(join(clientConsumer, 'package.json'), { name: '@open-harness/oh-client-consumer' })
