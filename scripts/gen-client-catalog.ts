...
const CLIENT_NOTES: readonly string[] = [
  ...
  PRIORITY_NOTE,
  'You cannot `import` anything, so the design-system components are out of reach: build markup with `React.createElement` and ship CSS through `styles.insert(css)`. Use the theme CSS variables (`var(--oh-alias-bg-layer-1)`, `var(--oh-alias-label-primary)`, …) instead of literal colors, or your contribution breaks in the other color scheme.',
...
/** Drop the `@open-harness/oh-` prefix so rows stay readable. */
function shortPackage(name: string): string {
  return name.replace('@open-harness/oh-', '')
}
...
export function renderClientCatalog(entries: readonly SlotEntry[]): string {
  const lines: string[] = [
    ...
    ' *',
    ' * @module @open-harness/oh-cordis-client-runner/client/slot-catalog',
    ' */',
    '',
    '/* jscpd:ignore-start */',
