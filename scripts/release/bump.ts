 ...
 *
 * The oh family shares one version across its members and the workspace root:
...
/** Release types the oh family accepts besides an explicit version. */
const RELEASE_TYPES = ['major', 'minor', 'patch'] as const

/** The workspace root manifest, which carries the oh family's version. */
...
function nextSharedVersion(current: string, request: string): string {
  if (!RELEASE_TYPES.includes(request as typeof RELEASE_TYPES[number])) {
    if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(request)) {
      throw new Error(`usage: release:oh <major|minor|patch|x.y.z>, got ${request}`)
    }
    return request
  }
...
/**
 * Plan the oh family's rewrite: one version for every member and the root.
 * @param family - the oh family.
 * @param root - repository root.
...
function main(): void {
  ...
  if (values.family === undefined) throw new Error('usage: bump.ts --family <oh|vendor> [version]')
  ...
  if (family.id === 'oh') {
    ...
    if (request === undefined) throw new Error('usage: release:oh <major|minor|patch|x.y.z>')
    if (values.prerelease !== undefined) {
      throw new Error('release:oh takes the prerelease in its version argument, as in 0.0.1-rc.1')
    }
