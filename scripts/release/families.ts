...
const WORKSPACE_ROOT_PACKAGE = 'open-harness'
...
export abstract class ReleaseFamily {
  ...
  members(root: string): ReleaseMember[] {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (name !== 'oh' && !name.startsWith('@open-harness/')) throw new Error(`${normalized} must name an @open-harness package or the oh CLI`)
...
class OhFamily extends ReleaseFamily {
  ...
  readonly installedEntry = { packageName: 'oh', binPath: 'lib/bin.js' }
}
...
class VendorFamily extends ReleaseFamily {
  ...
  tagPrefixFor(member: ReleaseMember): string {
    return `${this.tagPrefix}${member.name.replace('@open-harness/', '')}-v`
  }
