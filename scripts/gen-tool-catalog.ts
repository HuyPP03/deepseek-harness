...
import SqliteSessionQueryEngine from '@open-harness/oh-session-query-sqlite'
import GoalService from '@open-harness/oh-goal'
import SystemPrompt from '@open-harness/oh-system-prompt'
import ToolRuntime, { type Config as ToolsConfig } from '@open-harness/oh-tools'
import LocalBashExecutor from '@open-harness/oh-bash-local'
import * as BashEnvPlugin from '@open-harness/oh-shell-env'
import { PwshLocalExecutor } from '@open-harness/oh-pwsh-local'
...
import PlanModeController from '@open-harness/oh-plan-mode'
import WebRuntime from '@open-harness/oh-web'
import * as WebSearchExa from '@open-harness/oh-web-search-exa'
import * as WebFetchLocal from '@open-harness/oh-web-fetch-http'
import SubagentRuntime from '@open-harness/oh-subagent'
import type { SubagentProvider, SubagentReportDelivery } from '@open-harness/oh-subagent'
import * as ToolSubagentControl from '@open-harness/oh-tool-subagent-control'
import * as ToolSubagentListAgents from '@open-harness/oh-tool-subagent-control/list-agents'
import * as ToolSubagentReport from '@open-harness/oh-tool-subagent-report'
import SkillRegistry from '@open-harness/oh-skill'
import * as SkillFileSystem from '@open-harness/oh-skill-filesystem'
import LocalJobRegistry from '@open-harness/oh-jobs-local'
import * as ToolAskUser from '@open-harness/oh-tool-ask-user'
import * as ToolBash from '@open-harness/oh-tool-bash'
import * as ToolPwsh from '@open-harness/oh-tool-pwsh'
import * as ToolBashPersistent from '@open-harness/oh-tool-bash-persistent'
import CordisHostRunner from '@open-harness/oh-cordis-host-runner'
import * as ToolCordis from '@open-harness/oh-tool-cordis'
import * as ToolFs from '@open-harness/oh-tool-fs'
import * as ToolFsSearch from '@open-harness/oh-tool-fs-search'
import * as ToolStrReplaceEditor from '@open-harness/oh-tool-str-replace-editor'
import TerminalSessionService from '@open-harness/oh-terminal'
import * as ToolPty from '@open-harness/oh-tool-terminal'
import * as ToolGoal from '@open-harness/oh-tool-goal'
import * as ToolSchedule from '@open-harness/oh-schedule'
import Lsp from '@open-harness/oh-lsp'
import * as ToolLsp from '@open-harness/oh-tool-lsp'
import * as ToolSkill from '@open-harness/oh-tool-skill'
import * as ToolSessionQuery from '@open-harness/oh-tool-session-query'
import * as ToolTasks from '@open-harness/oh-tool-jobs'
import * as ToolTodo from '@open-harness/oh-tool-todo'
import * as ToolSubagent from '@open-harness/oh-tool-subagent'
import * as ToolWeb from '@open-harness/oh-tool-web'
import VmWorkflowEngine from '@open-harness/oh-workflow-worker-thread'
import * as ToolRalph from '@open-harness/oh-tool-ralph'
import * as ToolWorkflow from '@open-harness/oh-tool-workflow'
...
const TOOL_PACKAGES: ToolPackage[] = [
  {
    pkg: '@open-harness/oh-tool-ask-user',
    dir: 'tool-ask-user',
  ...
  },
  {
    ...
    note:
      'The bash tool is the model-facing consumer of the bash executor seam. A `run_in_background` run registers with the generic `ctx.jobs` runtime and is collected/stopped through the `job_*` tools from `@open-harness/oh-tool-jobs`; the `enableRunInBackground` config (default true) removes the parameter entirely when disabled.',
  },
  {
    pkg: '@open-harness/oh-tool-pwsh',
    dir: 'tool-pwsh',
    ...
    note:
      'The pwsh tool is the PowerShell-dialect consumer of the bash executor seam for Windows compositions (a PowerShell executor such as `@open-harness/oh-pwsh-local` backs `ctx.shell`); it mirrors the bash tool call-for-call minus sandbox controls — `run_in_background` runs register with the generic `ctx.jobs` runtime and are collected/stopped through the `job_*` tools, and the managed `OH_*` environment comes from `@open-harness/oh-shell-env`. Each call runs in a fresh process (no persistent PTY session), with native `C:\\...` paths and `$env:NAME` variables.',
  },
  {
    pkg: '@open-harness/oh-tool-cordis',
    dir: 'tool-cordis',
    ...
    note:
      'Not in any shipped tree (a deliberate opt-in — dynamic package code reaches the real runtime, see .agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.md). The toolset injects `ctx.dynamicCordisRunner` from `@open-harness/oh-cordis-host-runner`, which owns the definition registry and the vm sandbox; a composition missing it never activates the tools. A running package may register ADDITIONAL model-visible tools until it is stopped, undefined, or Open Harness restarts; a full changed request header logs those tool-set changes.',
  },
  {
    pkg: '@open-harness/oh-tool-bash-persistent',
  ...
  {
    pkg: '@open-harness/oh-tool-str-replace-editor',
  ...
  },
  {
    ...
    note:
      'The read-before-write/edit policy is added by `@open-harness/oh-fs-observation-policy` (an `fs/*` event-gate plugin, no schema change); a deployment that loads these tools is expected to also load it. `read_image` is not registered without `ctx.attachments`; its schema is route-independent, and execution refuses unless the exact routed model declares image input.',
  },
  {
    pkg: '@open-harness/oh-tool-fs-search',
    dir: 'tool-fs-search',
  ...
  {
    pkg: '@open-harness/oh-tool-terminal',
    dir: 'tool-terminal',
  ...
  },
  {
    ...
    note:
      'The lsp tool keeps provider selection and language-server subprocesses behind ctx.lsp, so its model-visible schema stays stable across providers. Requires a registered provider (e.g. `@open-harness/oh-lsp-stdio`) at runtime; without one, a query returns the structured `LSP_UNAVAILABLE` error rather than changing the schema.',
  },
  {
    pkg: '@open-harness/oh-tool-ralph',
    dir: 'tool-ralph',
  ...
  {
    pkg: '@open-harness/oh-tool-skill',
    dir: 'tool-skill',
    ...
    async mount(ctx) {
      ...
      await ctx.plugin(SkillFileSystem, {
        dshHome: resolve(root, '.tmp/tool-catalog/.oh'),
  ...
  {
    pkg: '@open-harness/oh-tool-session-query',
  ...
  {
    pkg: '@open-harness/oh-tool-subagent',
    dir: 'tool-subagent',
  ...
  {
    pkg: '@open-harness/oh-tool-subagent-control',
  ...
  {
    pkg: '@open-harness/oh-tool-subagent-report',
  ...
  {
    pkg: '@open-harness/oh-tool-workflow',
    dir: 'tool-workflow',
