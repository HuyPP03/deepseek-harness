...
const SESSION_PACKAGE = '@open-harness/oh-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@open-harness/oh-session/types'
 ...
 * top-level declaration (in `@open-harness/oh-session`) and any declaration
 * merge inside a `declare module '@open-harness/oh-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @open-harness/oh-session/known-event-types',
    ' */',
    '',
    '/**',
