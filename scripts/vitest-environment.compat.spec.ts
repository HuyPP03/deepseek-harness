...
describe('Vitest jsdom compatibility', () => {
  it('provides isolated browser storage instead of Node process storage', () => {
    ...
    localStorage.setItem('oh-vitest-storage-probe', 'available')

    expect(localStorage.getItem('oh-vitest-storage-probe')).toBe('available')
    localStorage.removeItem('oh-vitest-storage-probe')
  })
})
