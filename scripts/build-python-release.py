...
SDK_DISTRIBUTION = "open-harness-sdk"
RUNTIME_DISTRIBUTION = "open-harness-runtime-bin"
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="oh-python-release-") as temporary:
        ...
        if args.package == "sdk":
            ...
            environment = None
            expected = output_dir / f"open_harness_sdk-{wheel_version}-py3-none-any.whl"
        else:
            ...
            environment = {"OH_RUNTIME_PLATFORM_TAG": platform_tag}
            expected = output_dir / f"open_harness_runtime_bin-{wheel_version}-py3-none-{platform_tag}.whl"
...
def stage_sdk(destination: Path, version: str) -> None:
    ...
    text, count = re.subn(
        r'"open-harness-runtime-bin==[^"]+"',
        f'"open-harness-runtime-bin=={version}"',
        pyproject.read_text(),
        count=1,
    )
    if count != 1:
...
def stage_runtime(destination: Path, version: str, executable: Path, executable_name: str) -> None:
    ...
    runtime_dir = destination / "src" / "open_harness_runtime" / "runtime"
