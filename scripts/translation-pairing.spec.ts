...
function gitSupportsObjectFormat(format: 'sha256'): boolean {
  const root = mkdtempSync(join(tmpdir(), 'oh-git-object-format-'))
  try {
...
describe('translation pairing snapshots', () => {
  it('stores exact uncommitted bytes for later recovery by object ID', () => {
    const root = mkdtempSync(join(tmpdir(), 'oh-translation-pairing-'))
    try {
  ...
  it('reads staged bytes independently of the working tree', () => {
    const root = mkdtempSync(join(tmpdir(), 'oh-translation-pairing-index-'))
    try {
      execFileSync('git', ['init', '--quiet', root], {
        env: { ...process.env, GIT_DEFAULT_HASH: 'sha1' },
      })
...
describe('translation scope discovery', () => {
  ...
  it.each([
    ...
    'python/sdk-runtime/src/open_harness_runtime/runtime/oh-jsonrpc-agent-macos-arm64/README.md',
    'python/sdk-runtime/src/open_harness_runtime/runtime/node/README.md',
