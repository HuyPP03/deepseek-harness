/**
 * `oh plugin --profile <name> <args...>` — profile plugin management as a
 * thin pnpm forwarder: initialize the profile on first use, run
 * `pnpm <args...>` in the profile directory, then reconcile the
 * `oh.profile.bundles` layer list against the installed state (a dependency
 * resolving to a package that declares `oh.bundle` joins the layer stack; a
 ...
 * gained its `oh.bundle` declaration in a newer version.
 * @module oh/plugin
 */
 ...
 * @returns true when the package manifest declares `oh.bundle`.
 */
function exportsPatch(packageName: string, profileDir: string): boolean {
  ...
  return manifest.oh?.bundle?.patch !== undefined
}

/**
 * Reconcile `oh.profile.bundles` against the installed state: pnpm has
 * already written the real installed names (so a git/path/tarball/alias spec
 * on the command line reconciles by its true package name) and materialized
 * the packages. A dependency that resolves to a `oh.bundle`-declaring
...
function reconcilePlugins(before: ProfileManifest, profileDir: string): void {
  ...
  const plugins = after.oh?.profile?.bundles ?? []
  let changed = false
  for (const packageName of dependencies) {
    ...
    if (isBundle && !plugins.includes(packageName)) {
      ...
      process.stderr.write(
        `${NAME}: warning: ${packageName} declares no oh.bundle — installed as a plain dependency, not a profile layer `
  ...
  if (!changed) return
  after.oh = { ...after.oh, profile: { ...after.oh?.profile, bundles: plugins } }
 ...
 * @param cwd - the directory `oh` was invoked from.
...
/**
 * Run one `oh plugin` invocation: init if needed, forward to pnpm, reconcile.
