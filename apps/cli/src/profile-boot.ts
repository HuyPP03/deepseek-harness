...
/**
 * The home-level user patch layer (`$OH_HOME/cordis.patch.yml`), applied
 * over every profile's own layer. Resolved per call, not at module load:
 * `$OH_HOME` may be set by the test or launcher after import.
...
/** The session-telemetry row id the OH_TELEMETRY_DISABLED switch targets. */
 ...
 * switch set.
 * @param disabledEnv - the raw `OH_TELEMETRY_DISABLED` value (`undefined` when unset).
...
interface ComposedProfile {
  ...
  /** The home-level user layer (`$OH_HOME/cordis.patch.yml`), applied after the profile's own. */
...
function composeProfile(
  ...
  const telemetryPatch = resolveTelemetryPatch(process.env.OH_TELEMETRY_DISABLED, rows.has(TELEMETRY_ROW_ID))
...
export async function runProfile(options: RunProfileOptions): Promise<{ ctx: Context; shutdown: ProcessShutdown }> {
  ...
  const ctx = await boot(NAME, rootConfig, structuredClone(allPatches(composed)), (hostCtx) => {
    ...
    hostCtx.provide(OH_LAUNCH_ENVIRONMENT_KEY, options.environment)
    ...
    try {
      ...
      if (ctx.get('hmr') === undefined) {
        if (ctx.get('timer') === undefined) {
          await ctx.loader.create({ name: '@open-harness/cordis-plugin-timer' })
        }
        await ctx.loader.create({ name: '@open-harness/cordis-plugin-hmr', config: { root: [] } })
      }
