/**
 * Commander adapter for the `oh` command line.
 *
 ...
 * `@open-harness/oh-cmdline`). Launcher flags therefore come first: the first
 * token this parser does not recognize starts the inner arguments, so
 * `oh --profile tui --resume abc` boots the tui profile with `--resume abc`,
 * and `oh --profile web -h` prints the web app's help, not this one's.
 *
...
const HELP_EXAMPLES = `
Examples:
  oh --profile web                           boot the web profile (same as: oh web)
  oh --profile headless "run the tests"      answer one task, print the result, and exit
  oh --profile tui --patch ./extra.yml       boot a custom profile with one extra overlay
  oh --profile tui --resume <session>        arguments after the launcher flags reach the app
  oh --profile web --help                    the web app's own flags and help
  oh plugin --profile tui add <package>      install a plugin into the tui profile
`
...
export function parseOhArgs(argv: readonly string[], version: string): OhInvocation {
  ...
  program
    ...
    .description('oh: boot an Open Harness profile — an ordered stack of plugin-bundle patch layers under your own overrides.')
    .addHelpText('after', HELP_EXAMPLES)
    .exitOverride()
    // The launcher's flags come first and end at the first token it does not
    // know; everything from there on belongs to the booted app, including
    // its -h. `oh -h` with no profile still prints this help, below.
    .helpOption(false)
    .allowUnknownOption()
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the booted profile\'s app (see: oh --profile <name> --help)')
    .option('--profile <name>', 'the profile under $OH_HOME/profiles to boot')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
    .option('--dump-config', 'print the composed profile tree and exit')
    .option('--dump-default-config', 'print the profile tree without its user layer or --patch overlays and exit')
    .action((args: string[], options: BootOptions & { profile?: string }) => {
      // With the app owning -h, the launcher's own help is what a bare
      // `oh -h` (no profile to hand it to) must print.
  ...
  web
    ...
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the web app (see: oh web --help)')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
  ...
  if (resolved === undefined) throw new Error('oh: no invocation resolved')
  return resolved
}
