...
describe('shipped connector catalog', () => {
  it('boots in the web profile and lists every shipped connector', { timeout: 120_000 }, async () => {
    ...
    try {
      ...
      expect(views.map(view => view.id)).toEqual(['confluence', 'figma', 'github', 'notion', 'slack'])
      for (const view of views) expect(view.state).toBe('unconfigured')
      // The self-hosted row advertises its URL requirement from the first read on.
      const confluence = views.find(view => view.id === 'confluence')
      expect(confluence).toMatchObject({ urlRequired: true })
