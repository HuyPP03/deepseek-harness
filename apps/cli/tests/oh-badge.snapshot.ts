...
const binScript = fileURLToPath(new URL('./fixtures/oh-badge/snapshot.ts', import.meta.url))
const configPath = fileURLToPath(new URL('./fixtures/oh-badge/cordis.yml', import.meta.url))
const defaultConfigPath = fileURLToPath(new URL('./fixtures/oh-badge/default.cordis.yml', import.meta.url))
const tsconfigPath = fileURLToPath(new URL('../../../tsconfig.json', import.meta.url))
const badgeAssetsPath = fileURLToPath(new URL('../../../packages/skill/skill-badge/assets/', import.meta.url))

describe('oh badge assembled snapshot', () => {
  it('advertises and loads the opt-in bundled skill through the shipped app', async () => {
    const disabled = await runLoaderSmoke({
      label: 'disabled oh badge skill snapshot',
      tempDirPrefix: 'headless-snapshot-oh-badge-disabled-',
      binScript,
    ...
    const enabled = await runLoaderSmoke({
      label: 'oh badge skill snapshot',
      tempDirPrefix: 'headless-snapshot-oh-badge-',
      binScript,
      libBinScript: binScript,
      configPath,
      tsconfigPath,
    ...
    expect(disabledSnapshot).toMatchInlineSnapshot(`
      {
        ...
        "result": {
          "content": [
            {
              "text": "Error: skill "oh-badge" is unknown or no longer available",
              "type": "text",
            },
          ],
          "error": {
            "message": "skill "oh-badge" is unknown or no longer available",
          },
          "isError": true,
        },
        "summary": null,
    ...
    expect(enabledSnapshot).toMatchInlineSnapshot(`
      ...
      <available_skills>
      - \`oh-badge\`: Add the official “powered by oh” badge to documents, pull requests, merge requests, and other content produced with Open Harness. Use whenever creating a pull request or merge request. Also use when the user asks for a oh badge, powered-by-oh attribution, or a reusable oh badge asset or snippet.
      </available_skills>
      ...
      A user may also invoke a skill directly; its <skill_content> block then appears in this conversation. Follow it, and do not call the \`skill\` tool again for that skill.
        ...
        "result": {
          "content": [
            {
              "text": "<skill_content name="oh-badge">
      <skill_resources>
      ...
      - Local PNG: [\`oh-badge.png\`](oh-badge.png), 726×120 source image; render at 121×20
      - Shields.io image URL: \`https://img.shields.io/badge/powered_by-oh-4D6BFE?style=flat-square&logo=deepseek&logoColor=white\`
      ...
      \`\`\`markdown
      [![](https://img.shields.io/badge/powered_by-oh-4D6BFE?style=flat-square&logo=deepseek&logoColor=white)](https://github.com/deepseek-ai/deepseek-harness)
      \`\`\`

      If attribution should not be linked, use:

      \`\`\`markdown
      ![](https://img.shields.io/badge/powered_by-oh-4D6BFE?style=flat-square&logo=deepseek&logoColor=white)
      \`\`\`
      ...
      - For Feishu and other systems that import remote images unreliably, upload \`oh-badge.png\` from this skill directory instead of generating another badge.
