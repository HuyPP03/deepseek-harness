 ...
 * @param {import('@open-harness/cordis').Context} ctx - loader-mounted test plugin context.
 */
export function apply(ctx) {
  const keepAlive = setInterval(() => {}, 60_000)
  process.stderr.write('oh-test: never-dispose ready\n')
  ctx.effect(() => async () => {
    clearInterval(keepAlive)
    const armFile = process.env.OH_TEST_SHUTDOWN_ARM_FILE
    if (armFile === undefined || !existsSync(armFile)) return
    process.stderr.write('oh-test: never-dispose started\n')
    await new Promise(() => {})
  })
}
