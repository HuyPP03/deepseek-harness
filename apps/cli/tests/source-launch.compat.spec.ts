...
/**
 * Keyless smoke for SOURCE `oh` execution: run `apps/cli/src/bin.ts`
 * with the exact production runtime vector (`node --import tsx/esm`, the
 * vector the root `oh` script invokes directly) and assert the
 ...
 * breaks this gate instead of every developer's `pnpm oh`; the built-bin
...
describe('oh SOURCE launcher (node --import tsx/esm)', () => {
  it('launches the source CLI without building', async () => {
    const rootPackage = JSON.parse(await readFile(new URL('../../../package.json', import.meta.url), 'utf8')) as {
      readonly scripts?: Record<string, string>
    }
    expect(rootPackage.scripts?.oh).toBe('node --import tsx/esm apps/cli/src/bin.ts')
  })
  ...
  it('boots the source entry and requires a profile', async () => {
    ...
    if (result.timedOut) {
      throw new Error(`oh source launch did not exit within 25s. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
