...
import { boot, healProfilesModuleFallback, loadOverlayPatches } from '@open-harness/oh-app-boot'
...
import type { PatchOptions } from '@open-harness/cordis-plugin-include'
import { afterAll, beforeAll, describe, expect, it } from 'vitest'
import { settingsNamespace } from '@open-harness/oh-settings'
import { resolveSessionPreset, SETTINGS_NAMESPACE } from '@open-harness/oh-agent-presets'
import { applyChildComposition, childSessionMeta } from '@open-harness/oh-subagent'
import { CallId } from '@open-harness/oh-llm'
import type {} from '@open-harness/oh-compaction-basic'
...
import type {} from '@open-harness/oh-token-meter'

const CONFIG_DIR = fileURLToPath(new URL('../config/', import.meta.url))
const REPO_ROOT = fileURLToPath(new URL('../../..', import.meta.url))
/** The shipped Web surface: the oh-base and oh-web-app bundle patches over an empty preset root. */
...
async function bootWeb(
  ...
  const patches: PatchOptions[] = [
    ...loadOverlayPatches('oh-test', BASE_PATCH),
    ...loadOverlayPatches('oh-test', WEB_PATCH),
    // The settings row defaults to `$OH_HOME/settings.yaml`. Left alone it
    ...
    // storage-json's root is anchored to the real $OH_HOME. Unpinned, this
    // file writes the developer's own `~/.oh/storages/` — and then reads it
    ...
    // developer's own `~/.oh/.preset` cannot change this test's outcome.
  ...
  return await boot('oh-test', rootConfig, patches, (bootCtx) => {
...
let ctx: Context
beforeAll(async () => {
  const settingsFile = join(await mkdtemp(join(tmpdir(), 'oh-web-presets-')), 'settings.yaml')
...
describe('the shipped Web composition', () => {
  ...
  it('merges the global skill layer into a preset agent\'s catalog, keeping local discovery preset-side', async () => {
    const proj = await mkdtemp(join(tmpdir(), 'oh-preset-skill-proj-'))
    await mkdir(join(proj, '.oh', 'skills', 'project-proof'), { recursive: true })
    await writeFile(join(proj, '.oh', 'skills', 'project-proof', 'SKILL.md'), [
      '---',
      'name: project-proof',
    ...
    try {
      ...
      expect((await ctx.skills.list({ cwd: proj })).map(skill => skill.name)).toEqual(['oh-badge'])
      ...
      expect(scoped).toContain('oh-badge')
  ...
  it('shows a minimal agent the global layer but no loader tool', async () => {
    ...
    try {
      ...
      expect((await ctx.skills.list({ scope: handle.agent })).map(skill => skill.name)).toContain('oh-badge')
...
describe('product subagent rows in user presets', () => {
  ...
  beforeAll(async () => {
    const root = await mkdtemp(join(tmpdir(), 'oh-product-presets-'))
    ...
    productCtx = await bootWeb(settingsFile, [
      { insert: [
        { id: 'subagent-codex', name: '@open-harness/oh-subagent-codex' },
        { id: 'subagent-claude-code', name: '@open-harness/oh-subagent-claude-code' },
      ] },
      {
        id: 'agent-presets',
        config: {
...
describe('a launcher that configures no writable root', () => {
  ...
  // roster derives `<ohHome>/.agent-presets` itself. `$OH_HOME` is pointed
  ...
  beforeAll(async () => {
    const home = await mkdtemp(join(tmpdir(), 'oh-preset-derived-'))
    ...
    const settingsFile = join(await mkdtemp(join(tmpdir(), 'oh-preset-derived-settings-')), 'settings.yaml')
    await writeFile(settingsFile, '{}\n')
...
describe('authoring a preset on the shipped composition', () => {
  ...
  beforeAll(async () => {
    userRoot = join(await mkdtemp(join(tmpdir(), 'oh-preset-authoring-')), 'profiles')
    const settingsFile = join(await mkdtemp(join(tmpdir(), 'oh-preset-authoring-settings-')), 'settings.yaml')
    await writeFile(settingsFile, '{}\n')
