 ...
 * web profile (oh-base + oh-web-app bundle patches, auto-initialized).
...
function runBuiltWeb(cwd: string): Promise<{ stdout: string; stderr: string; code: number }> {
  return new Promise((resolveRun, rejectRun) => {
    const env: NodeJS.ProcessEnv = {
      ...process.env,
      DEEPSEEK_API_KEY: 'oh-cli-smoke-dummy-key',
      OH_HOME: join(cwd, '.oh'),
    }
    ...
    child.stdout.on('data', (chunk: string) => {
      stdout += chunk
      if (!settled && /oh web: http:\/\/127\.0\.0\.1:\d+/u.test(stdout)) {
        settled = true
        child.kill('SIGTERM')
      }
    })
...
describe.skipIf(!requireBuiltArtifacts)('built CLI lazy-search startup', () => {
  it('boots and disposes the shipped composition with full-text search off by default', async () => {
    ...
    const cwd = await mkdtemp(join(tmpdir(), 'oh-cli-lazy-search-'))
    try {
      const result = await runBuiltWeb(cwd)
      expect(result.stdout).toMatch(/oh web: http:\/\/127\.0\.0\.1:\d+/u)
