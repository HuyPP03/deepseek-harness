...
import { parseOhArgs } from '../src/args.ts'

const parse = (argv: string[]) => parseOhArgs(argv, '1.2.3')
...
describe('parseOhArgs', () => {
  ...
  it('routes the plugin pnpm forwarder', () => {
    ...
    expect(parse(['plugin', '--profile', 'tui', 'why', '@open-harness/cordis']))
      .toEqual({ mode: 'plugin', profile: 'tui', args: ['why', '@open-harness/cordis'] })
