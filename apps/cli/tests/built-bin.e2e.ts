...
import { startMockLlmServer } from '@open-harness/oh-llm-mock-server'
 ...
 * `@open-harness/oh-cmdline` and `commander` through the profile module
...
describe.skipIf(!existsSync(ohBin))('dsh BUILT bin (node lib/bin.js, no tsx)', () => {
  it('requires --profile and rejects removed commands', async () => {
    ...
    expect(help.stdout).toContain('oh --profile web')
    expect(help.stdout).toContain('oh plugin --profile')
  ...
  it('routes help and usage errors without activating startup-dependent rows', async () => {
    ...
    try {
      ...
      expect(web.stdout).toContain('Usage: oh --profile web')
  ...
  it('activates a dependency that gained oh.bundle in a later update', async () => {
    ...
    try {
      ...
      expect(manifest.oh.profile.bundles).toEqual(['@open-harness/oh-base', 'late-bundle'])
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, 30_000)
  ...
  describe('config dump', () => {
    ...
    it('prints the web profile bundle layers without a user layer', async () => {
      ...
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@open-harness/oh-agent-loop'")
      expect(stdout).toContain('agents: []')
      expect(stdout).toContain('# == @open-harness/oh-base')
      expect(stdout).toContain("name: '@open-harness/oh-host-webserver'")
    }, 30_000)
