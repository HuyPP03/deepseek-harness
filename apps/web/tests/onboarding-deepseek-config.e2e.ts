...
describe.skipIf(MODE === 'record')('web e2e: first-run DeepSeek credential setup', () => {
  ...
  beforeAll(async () => {
    scaffold = await launchWebScaffold({ deepSeekMissingCredential: true, welcomeNoticePending: true, localePreference: 'zh' })
  ...
  it('stores a key write-only and observes configured state without restarting', async () => {
    ...
    const secret = `oh_onboarding_${randomBytes(12).toString('hex')}`
