// Real-host smoke: spawn `oh web` with a real key, walk the full flow
...
// Readiness gate: `oh web` serves every production manifest plugin; until every UI
    ...
    const child = spawn(
      ...
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--port', '0'],
      {
        ...
        env: {
          ...
          OH_HOME: join(sessionsDir, '.oh'),
          OH_AGENTS_HOME: join(sessionsDir, '.agents'),
    ...
    const child = spawn(
      ...
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--port', '0'],
      {
        ...
        env: {
          ...
          OH_HOME: join(workspace, '.oh'),
          OH_AGENTS_HOME: join(workspace, '.agents'),
          TSX_TSCONFIG_PATH: join(REPO_ROOT, 'tsconfig.json'),
        },
        stdio: ['ignore', 'pipe', 'pipe'],
      },
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY || notReady.length > 0)('web smoke (real host, real key)', () => {
  ...
  beforeAll(async () => {
    ...
    // tsx boot mirrors the runtime half of the root oh script. Isolate
  ...
  it('empty-state first send completes a real model round', async () => {
    ...
    // This scenario spawns its own server against a fresh $OH_HOME with the
    ...
    await page.waitForFunction(
      () => document.title !== 'Open Harness' && document.title.endsWith(' — Open Harness'),
      undefined,
      { timeout: 15_000 },
    )
    ...
    await page.waitForFunction(
      expected => document.title === `${expected} — Open Harness`,
      durableTitle,
      { timeout: 15_000 },
    )
  ...
  it('dark mode: the body attribute cascades the token sheets', async () => {
    ...
    const dark = await page.evaluate(() => {
      document.body.setAttribute('data-oh-dark-theme', '')
