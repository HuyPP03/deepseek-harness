...
describe('web e2e: settings modal and General preferences', () => {
  ...
  it('opens the settings dialog, switches sections, and closes by every path', async () => {
    ...
    await dialog.getByRole('button', { name: 'Write Only' }).waitFor({ timeout: 10_000 })
  ...
  it('stores Permission as the default for future sessions without changing an existing session', async () => {
    ...
    const dialog = page.getByRole('dialog', { name: '设置' })
    await dialog.waitFor({ timeout: 10_000 })
    const selector = dialog.getByRole('button', { name: 'Write Only' })
  ...
  it('uses the persisted dark preference while plugins are still loading', async () => {
    ...
    try {
      ...
      const state = await loading.evaluate((element) => {
        ...
        return {
          attr: document.body.hasAttribute('data-oh-dark-theme'),
    ...
    await expect.poll(() => page.evaluate(() => document.body.hasAttribute('data-oh-dark-theme')), {
      timeout: 5_000,
    }).toBe(false)
    await page.keyboard.press('Escape')
    expect(tripwire.pageErrors).toEqual([])
  ...
  it('flips the theme through the Appearance cubes and persists across reload and a distinct port', async () => {
    ...
    const readState = async (target: Page = page): Promise<ThemeState> => await target.evaluate(() => {
      ...
      return {
        ...
        legacy: localStorage.getItem('oh.theme'),
        themeColor: metas[0]?.content ?? null,
        themeColorCount: metas.length,
        token: computed.getPropertyValue('--oh-alias-bg-base').trim(),
      }
    })
  ...
  it('persists the busy-state Enter behavior across reload and a distinct port', async () => {
    ...
    expect(await page.evaluate(() => localStorage.getItem('oh.conversation.busyEnter'))).toBeNull()
    await expect.poll(async () => readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8'), { timeout: 5_000 })
    ...
    try {
      ...
      expect(await secondPage.evaluate(() => localStorage.getItem('oh.conversation.busyEnter'))).toBeNull()
      expect(secondTripwire.pageErrors).toEqual([])
      expect(secondTripwire.warnings).toEqual([])
    } finally {
      await secondPage.close()
  ...
  it('persists the settings language across reload and a distinct port', async () => {
    ...
    try {
      ...
      expect(await secondPage.evaluate(() => localStorage.getItem('oh.locale'))).toBeNull()
      expect(secondTripwire.pageErrors).toEqual([])
      expect(secondTripwire.warnings).toEqual([])
    } finally {
      await secondPage.close()
  ...
  it('opens a Chinese browser in English without any stored preference', async () => {
    ...
    try {
      ...
      expect(await enPage.evaluate(() => localStorage.getItem('oh.locale'))).toBeNull()
