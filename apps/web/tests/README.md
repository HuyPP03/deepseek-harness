...
the deliberate composition divergences from `oh web` — are documented in
...
## Do not import `@open-harness/oh-client-*` here
...
`@open-harness/oh-goal/remote`, so the Host build phase ended up waiting on an
...
it imports `AppWebEntry` from `@open-harness/oh-client-web` and the boot-manifest
type from `@open-harness/oh-client-modules/client`: booting the real shell is what
...
`@open-harness/oh-client-runtime/client` because `client/runtime` is reachable
