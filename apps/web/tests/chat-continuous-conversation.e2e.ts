...
import { CallId, type StreamChunk } from '@open-harness/oh-llm'
import type { ReplayEntry, ReplayOverrideDoc } from '@open-harness/oh-llm-replay'
...
import { connectFreshWorkspace, conversationContextKey, newEnglishPage, saveFailureShot, stripLandlockInfoLine } from './support.ts'
...
describe('web e2e: continuous conversation grown through the composer', () => {
  ...
  beforeAll(async () => {
    replayDir = await mkdtemp(join(tmpdir(), 'oh-continuous-chat-replay-'))
  ...
  it.skipIf(MODE === 'record')('keeps twelve generated turns and tool rows bound to one live session', async () => {
    ...
    for (const spec of specs) {
      ...
      expect(stripLandlockInfoLine(toolResultText(results[0]!))).toBe(`${spec.toolResultMarker}\n`)
