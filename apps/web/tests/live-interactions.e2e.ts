...
// The model adapter is oh-llm-replay with override sidecars: `hang` (+ a
...
import { deriveReplayScript, parseSessionLog } from '@open-harness/oh-llm-replay'
import type { ReplayOverrideDoc } from '@open-harness/oh-llm-replay'
...
describe('web e2e: live-turn interactions (cancel / error / retry)', () => {
  ...
  async function launch(buildOverride?: (sidecarHome: string) => ReplayOverrideDoc): Promise<void> {
    ...
    if (buildOverride !== undefined) {
      ...
      sidecarDir = await mkdtemp(join(tmpdir(), 'oh-web-e2e-sidecar-'))
