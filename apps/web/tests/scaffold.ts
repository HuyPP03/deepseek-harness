...
// Boots the REAL web composition — the oh-base and oh-web-app bundle
...
// downlink, api-gateway, agent loop, tools, and persistence. Modes ride $OH_SNAPSHOT:
// replay (default, keyless: normally disables the llm-deepseek row and
// inserts oh-llm-replay in providers mode), record (real adapter + key,
...
//
// Composition divergences from `oh web`, all deliberate, all via include
...
import Group from '@open-harness/cordis-plugin-group'
import { scrubRequestHeaders, stabilizeFixtureMessageIds } from '@open-harness/oh-acp-snapshot'
import {
  assertEntriesLoaded,
  composeEntries,
...
import type { ReplayHandle } from '@open-harness/oh-llm-replay'
import { installLlmReplay, parseSessionLog } from '@open-harness/oh-llm-replay'
import SessionStore, {
  packChunkRuns,
...
/** Snapshot mode for the lane, from $OH_SNAPSHOT (same vocabulary as the other snapshot suites). */
...
export function webSnapshotMode(): WebSnapshotMode {
  const value = process.env.OH_SNAPSHOT
  if (value === undefined || value === '' || value === 'replay') return 'replay'
  if (value === 'record' || value === 'refresh') return value
  throw new Error(`OH_SNAPSHOT must be replay, record, or refresh; got ${JSON.stringify(value)}`)
}

/** The shipped composition under test: the oh-base and oh-web-app bundle patches over the empty profile root. */
...
export interface WebScaffold {
  ...
  /** Isolated harness home the settings/credentials rows write ($OH_HOME double). */
  harnessHome: string
...
export interface LaunchOptions {
  ...
  /**
   * Replay fixture (session.jsonl) served by the inserted oh-llm-replay row
...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  const workspaceCwd = await realpath(await mkdtemp(join(tmpdir(), 'oh-web-e2e-ws-')))
  // Isolated harness home: the settings/credentials rows resolve $OH_HOME
  // paths at load, and an in-process boot must NEVER touch the developer's
  // real ~/.oh document or credential file.
  const harnessHome = options.harnessHome ?? join(workspaceCwd, '.oh-home')
  ...
  // session is created. Without this a developer's real ~/.oh/skills silently
  // enters replay requests and goldens while CI sees none. `OH_HOME` follows
  ...
  const skillRootEnvironment = {
    OH_HOME: harnessHome,
    OH_AGENTS_HOME: join(workspaceCwd, '.agents-home'),
    OH_BUNDLED_SKILL_DIR: join(workspaceCwd, '.bundled-skills'),
  }
  ...
  try {
    persistenceRoot = await mkdtemp(join(tmpdir(), 'oh-web-e2e-sessions-'))
  } catch (error) {
  ...
  const patches: PatchOptions[] = [
    ...
    // Only the shipped root: a developer's own `~/.oh/.agent-presets` must not be
    ...
    // storage-json's yml root is anchored to the real $OH_HOME; pin the row
    // to an absolute temp root (removed with the workspace at close) so tests
    // never write the user's harness home.
    { id: 'storage-json', config: { root: join(workspaceCwd, '.oh-storages') } },
    // Skill discovery is model-visible input. Pin every host-level root inside
    // the owned temp world so ~/.oh, ~/.agents, and a bundled-root env setting
    ...
    // to the production OTLP endpoint (or whatever OH_TELEMETRY_OTLP_URL
    ...
    // (apps/web IS @open-harness/oh-web-frontend); only the URL line is silenced.
    ...
    { insert: [
      { id: 'directory-picker-browse', name: '@open-harness/oh-host-directory-picker-browse' },
      { id: 'ui-directory-picker-browse', name: '@open-harness/oh-client-ui-directory-picker-browse' },
    ] },
    ...
    ...options.cordisTools === true
      ? [{ insert: [
        { id: 'tool-cordis', name: '@open-harness/oh-tool-cordis' },
      ] }]
      : [],
  ...
  try {
    ...
    ctx.provide('ohHomePath', ohHomePath)
    ...
    // `@open-harness/cordis-plugin-group` by name.
...
export async function compareOrRefreshGolden(goldenPath: string, actual: string, mode: WebSnapshotMode): Promise<void> {
  ...
  if (!existsSync(goldenPath)) {
    throw new Error(`missing golden ${goldenPath} — run OH_SNAPSHOT=refresh pnpm run test:web to generate it`)
  }
