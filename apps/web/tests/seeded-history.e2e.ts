...
// fixture under the same record discipline as every other: OH_SNAPSHOT=record drives the turn
...
import type { ContentBlock, Message } from '@open-harness/oh-llm'
import { deriveEventMessage, SessionId } from '@open-harness/oh-session'
...
describe('web e2e: seeded history renders through cold resume', () => {
  ...
  it.skipIf(MODE === 'record')('an Access-chip switch lands one command row: bare name, non-repeating settlement text', async () => {
    ...
    await page.getByRole('button', { name: 'Access mode, current: Write Only' }).click()
  ...
  it.skipIf(MODE === 'record')('reports full feedback correlation ids in an expandable two-line row', async () => {
    ...
    process.env.OH_HOME = scaffold.harnessHome
    try {
      const input = page.locator('textarea').first()
