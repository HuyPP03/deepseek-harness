...
import open_harness_runtime as runtime
import pytest

from open_harness_runtime import (
...
def test_default_config_is_shipped_with_the_package() -> None:
    ...
    assert "@open-harness/oh-agent-spine-demo" in config
    assert "@open-harness/oh-session-persistence-jsonl" in config
    assert "@open-harness/oh-session-checkpoint-policy" in config
