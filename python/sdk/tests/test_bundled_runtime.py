...
from open_harness import DeepSeekHarness, HarnessClient, HarnessConfig
from open_harness.errors import TransportClosedError
...
- id: sdk-jsonrpc-server
  name: '@open-harness/oh-sdk-jsonrpc-server'
- id: agent-core
  name: '@open-harness/oh-agent-spine-demo'
  config:
    workspaceContext: false
- id: sessions
  name: '@open-harness/oh-session-persistence-jsonl'
  config:
    root: './sessions'
- id: session-checkpoints
  name: '@open-harness/oh-session-checkpoint-policy'
- id: subprocess
...
def _client(tmp_path: Path, launch_args: tuple[str, ...]) -> HarnessClient:
    return HarnessClient(
        HarnessConfig(
            ...
            cwd=str(tmp_path),
            env={
                "OH_CORDIS_CONFIG": "./cordis.yml",
                "OH_SESSION_ROOT": str(tmp_path / "sessions"),
                "OH_CWD": str(tmp_path),
...
def test_bundled_runtime_boots_a_cordis_config(tmp_path: Path, mode: str) -> None:
    ...
    assert init.serverInfo.name == "open-harness-sdk-runtime"
...
def test_python_sdk_boots_minimal_jsonrpc_config(tmp_path: Path, mode: str) -> None:
    ...
    harness = DeepSeekHarness(
        ...
        env={
            "OH_MODEL": model,
            "OH_CONTEXT_WINDOW": "1000000",
            "OH_SYSTEM_PROMPT": "You are the Python SDK minimal boot test agent.",
        },
...
def test_bundled_runtime_surfaces_unbundled_plugin_failure(tmp_path: Path, mode: str) -> None:
    ...
    (tmp_path / "cordis.yml").write_text(
        "- id: missing\n  name: '@open-harness/oh-does-not-exist'\n"
    )
    ...
    assert "@open-harness/oh-does-not-exist" in str(excinfo.value)
...
def test_zero_config_run_injects_bundled_default_cordis_config(
    ...
    monkeypatch.setenv("OH_RUNTIME_MODE", mode)
