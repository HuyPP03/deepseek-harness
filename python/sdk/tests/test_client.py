...
json.dump({
    ...
    "OH_CWD": os.environ.get("OH_CWD"),
    "OH_SESSION_ROOT": os.environ.get("OH_SESSION_ROOT"),
    "OH_CORDIS_CONFIG": os.environ.get("OH_CORDIS_CONFIG"),
}, open(env_dump, "w"))
...
""".strip()
    ...
    assert dumped_env["OH_CWD"] == str(tmp_path)
    assert dumped_env["OH_SESSION_ROOT"] == str(tmp_path / "sessions")
    assert dumped_env["OH_CORDIS_CONFIG"] == str(tmp_path / "cordis.yml")
...
for line in sys.stdin:
    ...
    if msg.get("method") == "initialize":
        json.dump({"process": os.getcwd(), "environment": os.environ.get("OH_CWD"), "wire": msg["params"]["cwd"]}, open(os.environ["CAPTURE"], "w"))
        print(json.dumps({"jsonrpc": "2.0", "id": msg["id"], "result": {"serverInfo": {"name": "fake-runtime"}}}), flush=True)
...
for line in sys.stdin:
    ...
    method = msg.get("method")
    if method == "initialize":
        print(json.dumps({"jsonrpc": "2.0", "id": msg["id"], "result": {"serverInfo": {"name": "fake-oh"}}}), flush=True)
    elif method == "session/prompt":
        params = msg.get("params") or {}
...
""".strip()
    ...
    with HarnessClient(
        ...
        assert init.serverInfo.name == "fake-oh"
...
def _install_fake_bundled_runtime(
    ...
    """
    runtime = tmp_path / "oh-jsonrpc-agent"
    runtime.write_text(
        """#!/usr/bin/env python3
import json
import os
...
json.dump({"OH_CORDIS_CONFIG": os.environ.get("OH_CORDIS_CONFIG")}, open(os.environ["ENV_DUMP"], "w"))
for line in sys.stdin:
    msg = json.loads(line)
    if msg.get("method") == "initialize":
...
def test_client_default_launch_uses_bundled_runtime_and_injects_default_config(
    ...
    if ambient_config is None:
        monkeypatch.delenv("OH_CORDIS_CONFIG", raising=False)
    else:
        monkeypatch.setenv("OH_CORDIS_CONFIG", ambient_config)
    ...
    assert json.loads(env_dump.read_text())["OH_CORDIS_CONFIG"] == str(default_config)
...
def test_client_respects_explicit_config_over_bundled_default(
    ...
    with HarnessClient(
        HarnessConfig(env={"ENV_DUMP": str(env_dump), "OH_CORDIS_CONFIG": "./explicit.yml"})
    ) as client:
        client.initialize(provider="deepseek-official", cwd="/workspace", model="deepseek-v4-pro")

    assert json.loads(env_dump.read_text())["OH_CORDIS_CONFIG"] == "./explicit.yml"
