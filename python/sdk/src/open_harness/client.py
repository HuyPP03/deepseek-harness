...
class HarnessConfig:
    """Configuration for launching the local Open Harness SDK runtime."""
...
class HarnessClient:
    """Synchronous JSON-RPC client for the Open Harness SDK runtime over stdio."""
    ...
    def close(self) -> None:
        ...
        try:
            ...
            proc.wait()
        self._proc = None
        self._fail_waiters(self._runtime_closed_error("Open Harness runtime closed"))
            ...
            while True:
                ...
                if deadline is not None:
                    ...
                    if remaining <= 0:
                        ...
                        raise TimeoutError(
                            f"{method} timed out waiting for Open Harness runtime{suffix}"
                        )
    ...
    def _write_message(self, message: JsonObject) -> None:
        ...
        if proc is None or proc.stdin is None:
            raise TransportClosedError("Open Harness runtime is not running")
        try:
            ...
            raise self._runtime_closed_error("Failed to write to Open Harness runtime") from exc

    def _start_reader_thread(self) -> None:
        self._reader_thread = threading.Thread(target=self._reader_loop, name="oh-runtime-reader", daemon=True)
        self._reader_thread.start()

    def _start_stderr_thread(self) -> None:
        self._stderr_thread = threading.Thread(target=self._stderr_loop, name="oh-runtime-stderr", daemon=True)
    ...
    def _reader_loop(self) -> None:
        ...
        try:
            ...
            self._fail_waiters(self._runtime_closed_error("Open Harness runtime stdout closed"))
