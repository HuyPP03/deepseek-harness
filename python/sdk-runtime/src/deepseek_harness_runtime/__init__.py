"""Locate the bundled Open Harness SDK runtime shipped with this package.
...
- **exe (production)**: single-file Node executables named
  ``oh-jsonrpc-agent-pkg-<platform>-<arch>`` (platform in {linux, macos}, arch in
...
- **node (dev-only)**: the full deploy closure under ``runtime/node/``
  ...
  runtime/node/node_modules/@open-harness/oh-sdk-jsonrpc-demo/lib/packaged-bin.js`` on a
...
the client SDK injects via ``$OH_CORDIS_CONFIG`` for zero-config runs — the
...
def bundled_default_config_path() -> Path:
    ...
    The client SDK injects this path via ``$OH_CORDIS_CONFIG`` when the caller
...
def resolve_bundled_launch_args(mode: str | None = None) -> tuple[str, ...]:
    ...
    ``OH_RUNTIME_MODE`` environment variable (``exe`` | ``node``), then
    ...
    raise ValueError(
        f"unsupported Open Harness runtime mode {selected!r}: expected 'exe' or 'node' "
...
def _current_platform_tag() -> str:
    ...
    if plat is None or arch is None:
        raise FileNotFoundError(
            "no bundled oh-jsonrpc-agent executable exists for this platform "
