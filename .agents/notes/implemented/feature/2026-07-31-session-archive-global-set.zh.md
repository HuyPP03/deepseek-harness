...
Sidebar workspace 浏览区的会话行菜单里，「Delete session」一直是纯视觉占位（无 handler）。产品口径定为**归档**而非删除：会话日志与 workspace 记账都不动，只把该会话从所有浏览视图（workspace 分组、聊天列表、搜索）里隐藏。归档记录需要一个落点：聊天列表的会话不属于任何 workspace 实体，per-workspace 字段放不下它。
...
- UI：菜单项 `delete`（visual-only）改为 `archive`（label「Archive session」，非 danger 样式，无确认对话框——非破坏性操作，误触后果只是列表隐藏）；过滤实现为 `tree.ts` 的 `sessionVisible` 判据加一档，`deriveGroups`/`deriveChats` 增加 `archived` 集合入参，三个视图（workspace 分组、聊天列表、搜索）同源生效。

## 已考虑的替代方案

**per-workspace archivedSessionIds（最初表述）。** 否决：聊天列表的会话没有 workspace 落点；用户改口全局。
