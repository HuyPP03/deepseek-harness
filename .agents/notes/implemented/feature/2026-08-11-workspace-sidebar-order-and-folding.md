...
The combined view menu offers **Manual** and **Last updated** in both browsing tabs ([Web chat sessions and the sidebar browsing tabs](2026-08-20-web-chat-sessions-and-sidebar-tabs.md)), with one browser-local persisted order per account. A real Workspace initializes from `WorkspaceView.sessionIds`; the ungrouped account (the Chats tab) initializes from recency and has no Host Session account. Entering Last updated performs one complete recency sort; a later user prompt or steer promotes that Session once, and dragging may edit the resulting order. Returning to Manual preserves the current order and only disables later activity promotion. Manual-mode drags for a real Workspace also write the Host Session account, while the chats drags and activity promotion remain browser-local. Chat rows omit an empty leading status slot because they have no parent hierarchy, while a visible status retains its slot.
...
- Workspace order is durable and shared through the Host, while grouping, open state, per-account Session view order, and query state remain browser-local presentation preferences. The chats list supports the same drag and promotion rules, but its order is browser-local because it has no Workspace account.
