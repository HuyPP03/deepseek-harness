...
- Coverage: the preview seats (including docx/xlsx/csv and the failure state) are unit-tested on committed fixtures; the e2e proves the seats end-to-end (raw-channel requests captured, sandboxed frames asserted) and the chat scenario asserts the real host fallback in-process through the scaffold's live context.
