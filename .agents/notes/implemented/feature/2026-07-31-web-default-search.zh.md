...
`apps/cli/config/base.cordis.yml` 明确挂载 `dsh-web`（带显式 `searchProvider`）、一个搜索提供方配置项，以及以 `fetch: false` 挂载的 `dsh-tool-web`。它不挂载 `dsh-web-fetch-http`，也不选择抓取提供方。共享 base 只将 `web_search` 设为 TUI、浏览器与无头会话的默认工具。显式搜索提供方 id 使选择不受注册顺序影响，同时个人覆盖层或 `--config` 覆盖层仍可替换或禁用这些配置项。已交付组合不再携带搜索超时覆盖；`dsh-tool-web` 提供方无关的 30 秒默认值同时适用于两项操作。

已交付的默认路由是无密钥的进程内 websift 提供方；DeepSeek 提供方是挂载的带密钥辅助选项。路由选择及其理由记录在 [websift 搜索路由决策](2026-08-22-web-search-websift.md)中。以下 DeepSeek 路由的机制描述在组合启用该选项时适用。

DeepSeek 路由使用与官方会话适配器相同的 `DEEPSEEK_API_KEY` 凭据引用。提供方在每次搜索内部通过可选的 `ctx.credentials` 服务解析该引用；只有未挂载该 seam 的组合才会回退到启动进程的环境变量，非空的 `apiKey` 字面值仍作为程序化配置的最后兜底。因此，由 Web 的 Models 页存储或轮换的密钥无需重启即可用于下一次搜索，提供方也无需保留该值。由于 `WebSearchProvider.available()` 是同步方法，它会将已安装解析器视为本地可用；若动态凭据缺失，操作会以提供方专属错误码 `WEB_PROVIDER_CREDENTIAL_MISSING` 失败，而稳定的工具 schema 仍保持注册。
...
**为已交付路由单独设置搜索超时覆盖。** 不予采纳：自定义提供方和部署有各自不同的延迟预期；`dsh-tool-web` 提供方无关的 30 秒默认值是这一预算的唯一归属，针对单个后端的覆盖值会成为第二个无法解释的数字。
...
每个已交付界面的原生模型请求都只会携带 `web_search` schema，以及仅用于搜索的提示词指引；Web／无头 Code Mode 通过 `run_code` 公开相同的搜索能力。该提示词要求模型使用返回的 snippet，且绝不会向模型提及已禁用的 `web_fetch` 工具。默认配置会提供搜索结果 snippet 与来源元数据，但不支持任意页面抓取；需要抓取完整页面的部署必须自行选择启用抓取。Web 快照通道会启动已交付配置树，经由真实默认提供方驱动一次回放的 `web_search` 调用，对照本地后端 double（模拟服务）执行，断言持久化的结构化结果，并固定最终浏览器呈现；当前默认路由及其通道记录在 [websift 搜索路由决策](2026-08-22-web-search-websift.md)中。TUI/Web 组合冒烟测试固定了共享的 `web_search` 清单及不提供 `web_fetch` 这一事实。DeepSeek 辅助路由的提供方测试固定缺失、已存储及已轮换凭据的行为，以及字面值与环境变量的兼容性。
