...
一个 seam、一条按平台的本地后端链、一个消费方，加上两个上层控制项：按调用的升级路径与按会话的运行时模式。以下所有内容均从叶子 `cordis.yml` 组合而来；不触及 `agent-loop`。跨工具族 fs 强制与按会话工作区根目录已经作为后续设计落到同一策略载体上；Windows 链已作为[Windows ACL 受限令牌沙箱](2026-08-08-windows-acl-restricted-token-sandbox.md)落地；剩余阶段——`subagent-acp` 消费方与更多环境——仍列在 § 延迟阶段。
...
fs/web/todo 在进程内执行，因此它们的沙箱语义是各自能力边界层面的策略。fs seam 现在通过沙箱提供方强制共享模式词汇（`dsh-fs-sandbox` 按模式限制 write/edit；见[跨工具族 fs 沙箱 RFC](2026-07-14-cross-family-fs-sandbox.md)），因此每个约束模式对文件系统工具都是真实边界，而非仅限 bash 的近似。web/todo 仍不受限制（web 的唯一效果是网络，不在文件效果模式词汇内）。没有通用的按工具沙箱运行时：主机中介的工具仅通过返回主机验证的声明式效果来离开进程，那是一次重写而非包装层——后续设计选择了一个共享策略归属 `ctx.sandboxPolicy`，由各能力强制，而不是统一包装层。
...
- **Windows 链**——已作为[Windows ACL 受限令牌沙箱](2026-08-08-windows-acl-restricted-token-sandbox.md)落地：`PLATFORM_CHAINS.win32` 挂载 `@deepseek-ai/dsh-sandbox-windows-acl`，其受限令牌方言、拒绝签名与 runner 失败规则记录在该笔记中；包装第三方 landstrip runner 的方案[经考虑后已驳回](../../rejected/feature/2026-07-26-evaluate-landstrip-for-windows-sandbox-rung.md)——它所经受的实战检验还不足以承载安全不变式。
...
- **在没有后端的平台上会发生什么？** `confine()` 抛出失败关闭的 `SANDBOX_UNAVAILABLE`，命令永不 spawn；今天每个受支持平台都有一条链（Linux：bwrap/Landlock，macOS：Seatbelt，Windows：[受限令牌 runner](2026-08-08-windows-acl-restricted-token-sandbox.md)），没有可用后端的主机仍按该失败关闭路径处理。
