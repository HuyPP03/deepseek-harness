...
Session lineage 不投影成列表层级。Workspaces 标签按 `WorkspaceView.sessionIds` 的手动序把源会话与所有 fork 子会话显示为同级行，每行都可独立打开、搜索和拖拽；Chats 标签在无手动顺序时按最近更新时间排序。`parentId` 仍用于 lineage 和后续查询，但不控制 session 列表可见性。
