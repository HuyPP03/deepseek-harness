...
interface SessionEventMap {
  'sandbox/mode': { mode: 'read-only' | 'workspace-write' | 'workspace-refs-write' | 'danger-full-access' }
...
fs/web/todo execute in-process, so their sandbox semantics are policy at their capability boundaries. The fs seam now enforces the shared mode vocabulary through a sandboxed provider (`dsh-fs-sandbox` fences write/edit by mode; see [the cross-family fs sandbox RFC](2026-07-14-cross-family-fs-sandbox.md)), so every confined mode is a real boundary for the filesystem tools, not a bash-only approximation. web/todo remain unfenced (web's only effect is network, outside the file-effect mode vocabulary). No generic per-tool sandbox runtime: a host-mediated tool leaves the process only by returning declarative effects the host validates, which is a rewrite, not a wrapper — the follow-up settled on one shared policy home (`ctx.sandboxPolicy`) with per-capability enforcement, not a uniform wrapper.
...
- **Windows chain** — shipped as [the Windows ACL restricted-token sandbox](2026-08-08-windows-acl-restricted-token-sandbox.md): `PLATFORM_CHAINS.win32` mounts `@deepseek-ai/dsh-sandbox-windows-acl`, whose restricted-token dialect, denial signatures, and runner-failure rules are recorded there; the landstrip wrapper was [considered and rejected](../../rejected/feature/2026-07-26-evaluate-landstrip-for-windows-sandbox-rung.md) — not battle-tested enough for a security invariant.
...
- **What happens on a platform with no backend?** `confine()` throws the fail-closed `SANDBOX_UNAVAILABLE` and the command never spawns; today every supported platform has a chain (Linux: bwrap/Landlock, macOS: Seatbelt, Windows: the [restricted-token runner](2026-08-08-windows-acl-restricted-token-sandbox.md)), and hosts without a usable backend still take that fail-closed path.
