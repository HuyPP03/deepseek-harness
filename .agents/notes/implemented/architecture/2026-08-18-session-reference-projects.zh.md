...
- **只读来自现行沙箱，而非新的检查。** 所有受限沙箱后端本来就允许读取会话工作区之外、只允许写入工作区之内，因此被接纳的引用目录可直接读取；包文档记录该保证及其已知缺口（`danger-full-access` 部署解除写限制——对一切皆如此）。
...
- 无密钥的 ACP 快照场景（`examples/acp-agent`，`reference-directories`，外加 `reject-extra-dirs` 拒绝对照）回放整个暴露面：会话日志中的事件与请求头中的提示词段。
...
- **只读工具或专门的引用读取工具。** 否决：文件工具本来就读取沙箱允许的整个文件系统；专门工具会重复它们，并为同样的访问给模型第二套词汇。
- **仅提示词上下文的实现（无事件）。** 被模型可见 ⟺ 已记录规则否决：该段必须能在 resume、fork 与 replay 时从日志重建出完全一致的内容。

## 影响

- **`danger-full-access` 部署解除引用的只读保证**，正如它解除会话工作区的一样；提示词文本仍告诉模型引用是只读的，但没有东西强制。已接受：该部署的策略声明已经公告了被解除的限制。
