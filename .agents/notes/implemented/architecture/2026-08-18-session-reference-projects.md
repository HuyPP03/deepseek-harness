# Agent Note: Session reference projects — read-only comparison projects attached to a session
...
- **Read-only comes from the standing sandbox, not a new check.** Every confined sandbox backend already allows reads outside the session workspace and writes inside it, so admitted reference directories are readable as-is; the package README records the guarantee and its known gap (a `danger-full-access` deployment lifts the write limit for everything).
...
- The keyless ACP snapshot scenarios (`examples/acp-agent`, `reference-directories` and the `reject-extra-dirs` denial pair) replay the full surface: the event in the session log and the prompt section in the request header.
...
- **A read-only tool or dedicated reference-reading tools.** Rejected: the file tools already read the whole sandbox-allowed filesystem; dedicated tools would duplicate them and give the model a second vocabulary for the same access.
...
- **`danger-full-access` deployments lift the references' read-only guarantee**, exactly as they lift the session workspace's; the prompt text still tells the model references are read-only, and nothing enforces it. Accepted: that deployment's policy statement already announces the lifted limit.
