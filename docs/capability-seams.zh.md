...
| `ctx.oauthTokens` | `core` | `oauth-tokens` | - | [`connectors`](../packages/connectors/connectors) | - | 位于 .connectors/oauth-tokens.json 的文件化、以 owner 为键的 OAuth 令牌包存储；连接器授权流程重新写入刷新后的令牌包或将其移除，消费方在每次使用时读取当前令牌包。 |
