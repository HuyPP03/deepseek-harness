...
flowchart LR
  ...
  svc_connectors["ctx.connectors<br/>Predefined external connections"]
  pkg_oauth_tokens["oauth-tokens"]
  svc_oauthTokens["ctx.oauthTokens<br/>OAuth token bundle store"]
  ...
  svc_workspaceReferences["ctx.workspaceReferences<br/>Session reference projects"]
  pkg_acp["acp"]
  ...
  pkg_oauth_tokens --> svc_oauthTokens
  ...
  pkg_subagent_sdk --> svc_subagents
  ...
  pkg_web_fetch_websift --> svc_web
  pkg_web_search_deepseek --> svc_web
  pkg_web_search_exa --> svc_web
  pkg_web_search_perplexity --> svc_web
  pkg_web_search_websift --> svc_web
  pkg_webserver --> svc_webServer
  pkg_workflow --> svc_workflowEngine
  pkg_workflow_worker_thread --> svc_workflowEngine
  pkg_workspace --> svc_workspaceRegistry
  pkg_workspace_references --> svc_workspaceReferences
  ...
  svc_connectors --> pkg_apiproxy
  svc_cordisInspect --> pkg_tool_cordis
  svc_credentials --> pkg_apiproxy
  svc_credentials --> pkg_llm_deepseek
  svc_credentials --> pkg_llm_pi_ai
  svc_credentials --> pkg_mcp_client
  ...
  svc_oauthTokens --> pkg_connectors
  svc_oauthTokens --> pkg_mcp_client
  ...
  svc_workspaceReferences --> pkg_acp
  svc_workspaceReferences --> pkg_apiproxy
...
| `ctx.typert` | `core` | [`typert-registry`](../packages/typert/registry) | - | [`typert-loader`](../packages/typert/loader), [`api-gateway`](../packages/api/gateway) | - | 插件直接或通过 oh-typert-loader 注册实时 zod 贡献；API 网关消费调用描述符和提供方，其他运行时消费方则在各自边界查询 schema 与反射元数据。 |
...
| `ctx.credentials` | `seam` | [`credentials`](../packages/credentials/credentials) | [`credentials-local`](../packages/credentials/credentials-local) | [`llm-deepseek`](../packages/llm/llm-deepseek), [`llm-pi-ai`](../packages/llm/llm-pi-ai), [`mcp-client`](../packages/mcp/mcp-client), `apiproxy` | - | 配置携带对机密信息的引用；提供方拥有实际值。消费方按操作解析，因此轮换后的凭据会在紧接着的下一次请求中生效；Web 网关提供不含实际值的视图和只写存储。 |
| `ctx.connectors` | `core` | [`connectors`](../packages/connectors/connectors) | - | `apiproxy` | - | MCP 服务器之上的 YAML 目录 + 按连接器的文档；从注册表、凭据与令牌 seam 派生 unconfigured→connected 状态，并通过 mcp-manager 挂载服务器。Web 网关提供不含机密信息的视图以及 configure/connect/add/remove 动词。 |
| `ctx.oauthTokens` | `core` | `oauth-tokens` | - | [`connectors`](../packages/connectors/connectors), [`mcp-client`](../packages/mcp/mcp-client) | - | 位于 .connectors/oauth-tokens.json 的文件化、以 owner 为键的 OAuth 令牌包存储；连接器授权流程重新写入刷新后的令牌包或将其移除，消费方在每次使用时读取当前令牌包。 |
...
| `ctx.workspaceReferences` | `core` | [`workspace-references`](../packages/workspace/workspace-references) | - | [`acp`](../packages/acp/acp), `apiproxy` | - | 整值 `workspace/references` 日志状态：附加到会话的只读项目目录，带 `workspace:references` 提示词段。 |
...
| `ctx.agentLoop` | `bundle` | [`agent-loop`](../packages/core/agent-loop) | - | [`agent-spine-demo`](../packages/examples/agent-spine-demo) | - | 唯一的具体循环插件；扩展包依赖 oh-agent 的事件和服务，而不依赖此包。 |
...
| `ctx.shellEnv` | `core` | [`shell-env`](../packages/shell/shell-env) | - | [`tool-bash`](../packages/shell/tool-bash), [`tool-pwsh`](../packages/shell/tool-pwsh) | - | 插件声明限定于 effect 作用域的 OH_* 事实；每个 shell 工具在每次执行时收集一份可信快照，其执行器据此重建命名空间。 |
...
| `ctx.subagents` | `seam` | [`subagent`](../packages/subagent/subagent) | [`subagent-spawn-in-process`](../packages/subagent/subagent-spawn-in-process), [`subagent-fork-in-process`](../packages/subagent/subagent-fork-in-process), [`subagent-acp`](../packages/subagent/subagent-acp), [`subagent-codex`](../packages/subagent/subagent-codex), [`subagent-claude-code`](../packages/subagent/subagent-claude-code), [`subagent-sdk`](../packages/subagent/subagent-sdk) | [`tool-subagent`](../packages/subagent/tool-subagent), [`tool-subagent-control`](../packages/subagent/tool-subagent-control), [`tool-ralph`](../packages/workflow/tool-ralph) | - | 提供方实现传输；该服务还负责可选的、基于 Activation 的延续编排，tool-subagent 选择一次性或可延续委派，tool-subagent-control 传递后续消息，而 tool-ralph 要求一条全新的结构化输出路由。 |
...
| `ctx.web` | `seam` | [`web`](../packages/web/web) | [`web-search-exa`](../packages/web/web-search-exa), [`web-search-perplexity`](../packages/web/web-search-perplexity), [`web-search-deepseek`](../packages/web/web-search-deepseek), [`web-search-websift`](../packages/web/web-search-websift), [`web-fetch-http`](../packages/web/web-fetch-http), [`web-fetch-websift`](../packages/web/web-fetch-websift) | [`tool-web`](../packages/web/tool-web) | - | 搜索和抓取提供方注册到同一个 ctx.web seam；tool-web 负责稳定的面向模型名称。 |
...
| `ctx.clientModules` | `core` | `modules` | - | `hmr` | - | 通过增量 `oh.client` 扫描组合 __OH_BOOT__ 入口图，提供插件组合包，并通知重建／图变更订阅方。 |
