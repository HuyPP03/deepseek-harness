...
| `ctx.oauthTokens` | `core` | `oauth-tokens` | - | [`connectors`](../packages/connectors/connectors) | - | A file-backed, owner-keyed store of OAuth token bundles at .connectors/oauth-tokens.json; the connector auth flow re-puts refreshed bundles or removes them, and consumers read the current bundle at each use. |
