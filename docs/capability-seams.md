...
| `ctx.credentials` | `seam` | [`credentials`](../packages/credentials/credentials) | [`credentials-local`](../packages/credentials/credentials-local) | [`llm-deepseek`](../packages/llm/llm-deepseek), [`llm-pi-ai`](../packages/llm/llm-pi-ai), [`mcp-client`](../packages/mcp/mcp-client), `apiproxy` | - | Configuration carries references to secrets; providers own the values. Consumers resolve per operation, so a rotated credential reaches the very next request; the web gateway exposes value-free views and write-only storage. |
| `ctx.connectors` | `core` | [`connectors`](../packages/connectors/connectors) | - | `apiproxy` | - | A YAML catalog plus per-connector documents over MCP servers; derives unconfigured→connected state from the registry, credential, and token seams, and mounts servers through mcp-manager. The web gateway exposes the secret-free views and the configure/connect/add/remove verbs. |
| `ctx.oauthTokens` | `core` | `oauth-tokens` | - | [`connectors`](../packages/connectors/connectors), [`mcp-client`](../packages/mcp/mcp-client) | - | A file-backed, owner-keyed store of OAuth token bundles at .connectors/oauth-tokens.json; the connector auth flow re-puts refreshed bundles or removes them, and consumers read the current bundle at each use. |
...
| `ctx.workspaceReferences` | `core` | [`workspace-references`](../packages/workspace/workspace-references) | - | [`acp`](../packages/acp/acp), `apiproxy` | - | Whole-value `workspace/references` log state: the read-only project directories attached to a session, with the `workspace:references` prompt section. |
...
| `ctx.agentLoop` | `bundle` | [`agent-loop`](../packages/core/agent-loop) | - | [`agent-spine-demo`](../packages/examples/agent-spine-demo) | - | The one concrete loop plugin; extension packages depend on oh-agent events and services, not on this package. |
...
| `ctx.shellEnv` | `core` | [`shell-env`](../packages/shell/shell-env) | - | [`tool-bash`](../packages/shell/tool-bash), [`tool-pwsh`](../packages/shell/tool-pwsh) | - | Plugins declare effect-scoped OH_* facts; each shell tool collects one trusted snapshot per execution and its executor rebuilds the namespace. |
...
| `ctx.web` | `seam` | [`web`](../packages/web/web) | [`web-search-exa`](../packages/web/web-search-exa), [`web-search-perplexity`](../packages/web/web-search-perplexity), [`web-search-deepseek`](../packages/web/web-search-deepseek), [`web-search-websift`](../packages/web/web-search-websift), [`web-fetch-http`](../packages/web/web-fetch-http), [`web-fetch-websift`](../packages/web/web-fetch-websift) | [`tool-web`](../packages/web/tool-web) | - | Search and fetch providers register into one ctx.web seam; tool-web owns the stable model-facing names. |
...
| `ctx.clientModules` | `core` | `modules` | - | `hmr` | - | Composes the __OH_BOOT__ entry graph from an incremental oh.client scan, serves plugin bundles, and notifies rebuilt/graph-changed subscribers. |
