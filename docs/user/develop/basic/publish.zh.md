...
前几篇教程通过 `--patch` overlay 加载本地插件。本教程把它打包成可安装的**组合包**（bundle），用 `oh plugin add` 安装进一个 **profile**，并解释决定组合后配置的层顺序。本文假设 `oh` CLI 已安装。请先完成[插件配置](./config.md)。

如果改用全新的源码 checkout，请先按照[从源码运行章节](../../../../README.md#run-from-source)完成准备，将本教程的 `hello-plugin` 目录放在仓库根目录，并从该目录把下文的 `oh ...` 命令改为 `pnpm oh ...`。构建与启动器行为见[源码执行](../../../../apps/cli/reference/README.md#source-execution)。
...
`oh plugin --profile <name> <args...>` 在 profile 目录内转发给 pnpm，因此所有 pnpm 子命令都可用。在包含 `hello-plugin` 的目录中安装该包的 checkout：

```sh
oh plugin --profile demo add ./hello-plugin
```

首次使用会初始化 profile（`@open-harness/oh-base` 作为它的第一个组合包），pnpm 链接该 checkout，而 `oh` 因为这个包声明了 `oh.bundle`，把它追加进 `oh.profile.bundles`：
...
1. profile 的 `oh.profile.bundles` 列表所列的各个组合包 patch，按列表顺序——先是 `@open-harness/oh-base`，然后是每个已安装组合包，按其加入顺序。
...
- **用户**为构建授权。pnpm ≥10 在得到显式允许之前拒绝运行 git 依赖的 `prepare` 脚本，所以第一次 `add` 会失败；`oh` 会指出修法——把 pnpm 打印的确切包键复制进该 profile 的 `pnpm-workspace.yaml`：
...
- **发布到 npm**，在 `pnpm publish` 时构建好 `lib/`；`oh plugin add your-package` 安装的就是预构建代码。
- **交付 tarball**：用 `pnpm pack` 打包；用户执行 `oh plugin add ./hello-plugin-0.1.0.tgz`。
