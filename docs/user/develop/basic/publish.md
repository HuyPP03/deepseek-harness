...
The previous tutorials loaded a local plugin through a `--patch` overlay. This tutorial packages it as an installable **bundle**, installs it into a **profile** with `oh plugin add`, and explains the layer order that determines the composed configuration. It assumes the `oh` CLI is installed. Complete [plugin configuration](./config.md) first.

To use a fresh source checkout instead, complete the [run-from-source section](../../../../README.md#run-from-source), keep this tutorial's `hello-plugin` directory at the repository root, and run the remaining `oh ...` commands from there as `pnpm oh ...`. See [source execution](../../../../apps/cli/reference/README.md#source-execution) for build and launcher behavior.
...
`oh plugin --profile <name> <args...>` forwards to pnpm in the profile directory, so every pnpm verb works. From the directory that contains `hello-plugin`, install the package checkout:

```sh
oh plugin --profile demo add ./hello-plugin
```

The first use initializes the profile (with `@open-harness/oh-base` as its first bundle), pnpm links the checkout, and `oh` appends the bundle to `oh.profile.bundles` because the package declares `oh.bundle`:
...
1. Each bundle patch named in the profile's `oh.profile.bundles` list, in list order — `@open-harness/oh-base` first, then each installed bundle in the order it was added.
...
```sh
oh plugin --profile demo add github:you/hello-plugin
```
...
- **The user** allowlists the build. pnpm ≥10 refuses to run a git dependency's `prepare` script until it is explicitly allowed, so the first `add` fails; `oh` points at the fix — copy the exact package key pnpm printed into the profile's `pnpm-workspace.yaml`:
...
- **Publish to npm** with `lib/` built at `pnpm publish` time; `oh plugin add your-package` then installs prebuilt code.
- **Ship a tarball** from `pnpm pack`; users run `oh plugin add ./hello-plugin-0.1.0.tgz`.
