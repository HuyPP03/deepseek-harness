...
- **Service Definition** (`oh-shell`) — defines the Cordis service and Bash request and result types
- **Service Provider** (`oh-bash-local`) — executes commands on the local machine
- **Consumer** (`oh-tool-bash`) — exposes the capability as a model-callable tool

```
┌─────────────┐     ┌──────────────────┐     ┌──────────────┐
│  oh-shell   │────▶│  oh-bash-local  │     │ oh-tool-bash│
...
import { MyCapService, type MyCapRequest, type MyCapResult } from '@open-harness/oh-my-cap'
...
```yaml
- name: '@open-harness/oh-my-cap-local'
- name: '@open-harness/oh-tool-my-cap'
```
