...
| [connectors.md](connectors.md) | predefined external connections: the connector manifest and `ConnectorState` state machine, the `connector/state` event, the passive `OAuthTokenBundle` store, and interim secret handling |
...
| [subprocess.md](subprocess.md) | the subprocess seam: fully-explicit `SubprocessSpawnSpec`, offset-based output readers, unclassified `SubprocessOutcome`, and the managed `OH_*` environment vocabulary |
...
| [client-modules.md](client-modules.md) | the web plugin table: `oh.client` declarations, `WebBootGraph` wire composition, the bundle route and index tap |
