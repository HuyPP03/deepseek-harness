...
- **Provider stalls are bounded at the transport.** Both shipping remote adapters expose positive finite `streamIdleTimeoutMs` with a five-minute default. The watchdog arms only while iterator `next()` is outstanding, uses one stable signal for the whole request, maps its own expiry to `TIMEOUT`, and keeps an earlier caller abort as `ABORTED`. The window is phase-aware: while the stream emits tool-call deltas (until the next text or reasoning delta), both adapters arm `toolCallStreamIdleTimeoutMs` instead — defaulting to `streamIdleTimeoutMs` — so a deployment whose server batches tool-call arguments can widen only that phase.
...
- **An empty completion is a retryable error, not a silent success.** Both adapters map a terminal `stop` finish that carried no content blocks to `finish {kind:'error'}` with the canonical `EMPTY_RESPONSE` code, and `oh-llm-retry` retries it by default; see [empty model responses are retryable](../../.agents/notes/implemented/bug-fix/2026-07-24-empty-model-response-is-retryable.md).
...
declare class BlockAssembler {
   ...
   */
  message(source: MessageSource = { kind: 'plugin', plugin: 'oh-llm/assembler' }): Message;
}
```
...
interface GenerateOptions {
   ...
   * the derived history (oh-agent-loop); a hand-built one-shot passes any list.
   */
  messages: Message[]
...
`GenerateOptions.tools` carries `ToolSchema` — the JSON-schema description of a tool, as sent to the model. It is declared in oh-llm (not oh-tools) precisely because it is part of the request the loop assembles every step:

```ts type-equiv
/**
 * JSON-schema description of a tool, as sent to the model.
 *
 * Declared here (not in oh-tools) because it is part of {@link GenerateOptions};
 * oh-tools' ToolDefinition and oh-system-prompt's PromptAssembly both import
