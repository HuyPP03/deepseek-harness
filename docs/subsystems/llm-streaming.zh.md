...
- **提供方停顿在传输层受到时限约束。** 两个已交付的远程适配器都暴露正数且有限的 `streamIdleTimeoutMs`，默认五分钟。watchdog 只在 iterator `next()` 尚未完成时启动，整个请求使用同一个稳定 signal，把自身到期映射为 `TIMEOUT`，并把更早发生的调用方中止保留为 `ABORTED`。窗口按阶段区分：当流正在发出 tool call delta（直到下一个 text 或 reasoning delta）时，两个适配器改为布防 `toolCallStreamIdleTimeoutMs`（默认值取 `streamIdleTimeoutMs`），因此服务器对 tool call 参数做批量处理的部署可以只加宽该阶段。
...
- **空 completion 是可重试错误，而不是静默的成功结果。** 两个适配器都把没有携带任何内容块的终止性 `stop` 结束映射为携带规范 `EMPTY_RESPONSE` code 的 `finish {kind:'error'}`，`oh-llm-retry` 默认会重试它；详见[空模型响应可重试](../../.agents/notes/implemented/bug-fix/2026-07-24-empty-model-response-is-retryable.md)。
...
`GenerateOptions.tools` 携带 `ToolSchema`——工具的 JSON Schema 描述，发送给模型。它声明在 oh-llm（而非 oh-tools）中，正是因为它是循环每一步组装请求的一部分：
