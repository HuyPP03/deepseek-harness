...
每个子系统一页，覆盖 Open Harness 的全部子系统：它是什么、它操作哪些数据结构，以及——当它由某个 `ctx` 服务或事件作用域支撑时——一段生成的 **Cordis API** 小节，承载其服务与事件参考。本目录与 [architecture.md](../architecture.md) 互补：后者描述跨子系统的*行为*（服务映射、会话/轮次/步骤生命周期、事件分类体系）；这里的每一页是单个子系统词汇与接线的参考。
...
| [connectors.md](connectors.md) | 预定义外部连接：连接器清单与 `ConnectorState` 状态机、`connector/state` 事件、被动的 `OAuthTokenBundle` 存储，以及临时密钥处理 |
...
| [subprocess.md](subprocess.md) | 子进程 seam：完全显式的 `SubprocessSpawnSpec`、基于偏移的输出读取器、不含分类的 `SubprocessOutcome`，以及受管 `OH_*` 环境词汇 |
...
| [client-modules.md](client-modules.md) | Web 插件表：`oh.client` 声明、`WebBootGraph` 线上组合、bundle 路由与 index 转换 |
